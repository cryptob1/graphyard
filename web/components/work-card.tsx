...
export default function WorkCard({ item, repository, now, onOpen }: { item: Work; repository?: string | null; now: number; onOpen(id: string): void }) {
  ...
  return <div className={`card tone-${status.tone}`} onClick={() => onOpen(item.id)}>
    ...
    <div className="card-meta"><p className="status-line" title={`In this step for ${age(item.stageEnteredAt)}`}><Explained sentence={status.sentence}/></p>{item.candidate && <span className="push card-pr"><CandidatePr repository={repository} candidate={item.candidate} workKey={item.key}/></span>}</div>
  </div>;
}
