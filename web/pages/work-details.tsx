...
import { groupOf, humanOnlyIds, nextActor, timedGroups } from '../groups';
...
export default function WorkDetails({ item, work, status, token, observedAt, jobs, queue, events, busy, codexAvailable, editingRequirements, setEditingRequirements, action, api, refresh, setSelected, setView, sessionEpoch }: Dashboard & { item: Work }) {
  ...
  const group = groupOf(item, now, humanOnlyIds(work, status?.humanOnly));
  ...
  const failing = item.gates.find(g => !g.passed);
  ...
  const check = (name: string) => item.observation?.checks?.find(c => c.name === name)?.result;
  const approved = (item.observation?.reviews ?? []).some(r => r.state === 'APPROVED' && r.sha === item.candidate?.sha);
  return <article className="item-page" aria-label={item.title}>
    ...
    <div className="item-columns">
      <div className="item-column">
        ...
        <section className="panel" aria-label="Requirements"><h2>Requirements <small>{item.criteria.length}</small></h2>
          {item.criteria.map(ac => <div className="criterion" key={ac.id}><span className="mono">{ac.id}</span> <span className="criterion-text" title={ac.text}>{oneLine(ac.text)}</span>
            <ul className="proof-markers">{proofs.filter(p => p.criterion === ac.id).map(p => <li key={p.proof} className={`marker-${marker[p.status]?.tone ?? 'pending'}`}>{marker[p.status]?.symbol ?? '○'} <Term term="proof">{p.proof}</Term> {marker[p.status]?.word ?? p.status}</li>)}</ul></div>)}</section>
      ...
      <div className="item-column">
        <section className="panel" aria-label="Pull request"><h2>Pull request</h2>
          {item.candidate ? <dl className="facts">
            ...
            <div><dt>Review</dt><dd>{!item.policy.review ? 'not required' : approved ? 'Approved' : 'Waiting for approval'}</dd></div>
