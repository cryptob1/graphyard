    ...
    <details className="more-details"><summary>More details</summary>
      ...
      {item.stage !== 'done' && current >= 0 && <><h3>Gate steps</h3><ol className="steps">
        {item.gates.map((g, i) => i < current ? <li key={g.name} className="step done">✓ {gateLabel[g.name] ?? g.name}</li> : i === current && <li key={g.name} className="step current">○ {gateLabel[g.name] ?? g.name}
          {g.name === 'acceptance' ? <p>{g.reasons.length} {g.reasons.length === 1 ? 'thing' : 'things'} still to prove.</p> : <ul>{g.reasons.map(r => <li key={r}>{shortShas(plainReason(r, g.name).text)}</li>)}</ul>}</li>)}
      </ol>
      {later.length > 0 && <details className="later-steps"><summary>Later steps ({later.length})</summary><ul>{later.map(g => <li key={g.name}><strong>{gateLabel[g.name] ?? g.name}:</strong> {shortShas([...new Set(g.reasons.map(r => plainReason(r, g.name).text))].join('; '))}</li>)}</ul></details>}</>}
