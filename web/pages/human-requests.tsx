...
export { researchQuestionRows, researchQuestionRule } from '../../src/model/human-request';
...
export default function HumanRequestsPage({ work, status, observedAt, action, busy, setSelected }: Dashboard) {
  ...
  // The rows the server derived from the rule table — the research brief's product questions
  // (GY-259) included, derived there beside it since GY-401, so every count or notification built
  // from the status read sees them too. A client whose status read has not landed yet still sees
  // every rule whose instances live on the work documents it already holds, derived here from
  // that same table; a server older than this page, whose list carries no research rows, is
  // patched from the work documents, deduplicated by request id.
  const derived: HumanRequestRow[] = status?.humanOnly ?? openHumanOnly(work.map(item => ({ work: item })), now);
  const known = new Set(derived.map(row => row.request.id));
  const requests: HumanRequestRow[] = [...derived, ...researchQuestionRows(work, now).filter(row => !known.has(row.request.id))]
