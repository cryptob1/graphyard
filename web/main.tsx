...
function App() {
  ...
  return <div className="shell"><aside className="sidebar"><div className="brand"><img className="mark" src="/graphyard-symbol.svg" alt="" width="32" height="32"/> graphyard</div><div className="workspace-label">CONTROL PLANE</div>{views.map(entry => (!entry.adminOnly || status?.actor?.role === 'admin') && <button key={entry.id} className={view === entry.id ? 'nav active' : 'nav'} onClick={() => entry.open ? entry.open(dashboard) : setView(entry.id)}>{entry.icon} <span>{entry.label}</span>{entry.count && <small>{entry.count(dashboard)}</small>}</button>)}<a className="nav" href="/docs">↗ <span>Documentation</span></a><div className="sidebar-bottom"><span className={`dot ${connected ? 'green' : ''}`}/>{connected ? 'Control plane connected' : 'Disconnected · data may be stale'}<p>{status?.actor?.id} · {status?.actor?.role} <SessionBadge kind={status?.actor?.sessionKind}/></p><button className="text-button" onClick={signOut}>Sign out</button></div></aside>
