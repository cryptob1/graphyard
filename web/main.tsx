...
function App() {
  ...
  function signOut() { sessionStorage.removeItem('graphyard-token'); setToken(''); setDraftToken(''); setStatus(null); setWork([]); setSelected(null); setEvents([]); setCreating(false); setConnected(false); setLastUpdated(null); setError(''); }
  async function refresh() { const [items, system] = await Promise.all([api('work'), api('status')]); setWork(items); setStatus(system); setConnected(true); setLastUpdated(new Date().toLocaleTimeString()); setError(''); }
  useEffect(() => {
    if (!token) return;
  ...
  return <div className="shell"><aside className="sidebar"><div className="brand"><span className="mark">g</span> graphyard</div><div className="workspace-label">CONTROL PLANE</div><button className={view === 'graph' ? 'nav active' : 'nav'} onClick={() => setView('graph')}>⌘ <span>Delivery graph</span></button><button className={view === 'board' ? 'nav active' : 'nav'} onClick={() => setView('board')}>▥ <span>Work board</span><small>{work.length}</small></button><button className={view === 'scenarios' ? 'nav active' : 'nav'} onClick={() => setView('scenarios')}>✓ <span>Test cases</span></button><a className="nav" href="/docs">↗ <span>Documentation</span></a><div className="sidebar-bottom"><span className={`dot ${connected ? 'green' : ''}`}/>{connected ? 'Control plane connected' : 'Disconnected · data may be stale'}<p>{status?.actor?.id}</p><button className="text-button" onClick={signOut}>Sign out</button></div></aside>
    ...
    {item && <Dialog onClose={() => setSelected(null)}><section role="dialog" aria-modal="true" aria-label={item.title} className="drawer" onClick={e => e.stopPropagation()}><button className="close" aria-label="Close details" onClick={() => setSelected(null)}>×</button><div className="eyebrow">{item.key} / {item.type}</div><h2>{item.title}</h2><p>{item.description}</p><div className="tags"><span>{item.stage}</span><span>Policy v{item.policyRevision}</span><span>Revision {item.revision}</span></div>{item.violations.map(v => <div className="notice danger" key={v}>{v}</div>)}<h3>Ownership</h3><p>{item.lease ? `${item.lease.owner} · lease ${item.lease.epoch} · expires ${new Date(item.lease.expiresAt).toLocaleTimeString()}` : 'No active assignment'}</p>{item.workspaces.map(w => <code key={w.epoch}>{w.host}:{w.path}<br/>{w.branch} · assignment {w.epoch}</code>)}{item.candidate && <p>PR #{item.candidate.pr} · <code>{item.candidate.sha.slice(0, 12)}</code></p>}
