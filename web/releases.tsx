...
import type { EnvironmentDelivery, Release } from '../src/delivery';
...
export default function ReleasesView({ api, work }: { api: (path: string) => Promise<any>; work: Work[] }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try { const data = await api('delivery'); if (!stopped) { setEnvironments(data.environments); setReleases(data.releases); setError(''); setLoaded(true); } }
