...
execFileSync('npx', [...railway, 'variable', 'set', '--service', 'graphyard', '--skip-deploys', 'DATABASE_URL=${{Postgres.DATABASE_URL}}', 'HOST=0.0.0.0', 'PORT=4310', 'GITHUB_REPOSITORY=cryptob1/graphyard', ...limits.lines], { stdio: ['ignore', 'ignore', 'inherit'] });
console.log(`Railway configured with ${limits.lines.join(' ')}${limits.drift.length ? ` (corrected ${limits.drift.map(entry => entry.variable).join(', ')})` : ''}. Credentials saved to .graphyard/credentials.json (mode 0600); no credentials printed. Redeploy the service for the variables to take effect.`);
