...
export interface DispatchEffects {
  ...
  reconcileReviews: (work: Work[]) => Promise<{ reviews: ReviewRecord[] }>;
...
export async function runDispatchTick(config: MasterConfig, cursor: DispatchCursor, effects: DispatchEffects, now: () => number = Date.now, readTimeoutMs = dispatchReadTimeoutMs): Promise<DispatchTick> {
  ...
  const { reviews } = await effects.reconcileReviews(snapshot.work);
  ...
  for (const item of snapshot.work.filter(work => work.stage !== 'done' && work.autoDispatch)) {
    ...
    if (review?.state === 'requested' && review.provider === 'github') {
      if (reviews.some(record => record.requestId === review.id)) tick.skipped++;
    ...
    for (const request of item.autoDispatch!.producers.filter(entry => entry.state === 'requested')) {
      if (producers.some(record => record.requestId === request.id)) { tick.skipped++; continue; }
...
export async function runAutoDispatch(config: MasterConfig, cursor: DispatchCursor, effects: DispatchEffects, options: { intervalMs: number; once?: boolean; signal?: AbortSignal; now?: () => number; log?: (line: string) => void; readTimeoutMs?: number; retryMinMs?: number }) {
  ...
  do {
    ...
    try {
      ...
      wait = tickRetryDelay(cursor.consecutiveFailures, options.intervalMs, options.retryMinMs);
...
export function dispatchEffects(root: string, config: MasterConfig, deps: { snapshot: () => Promise<{ work: Work[]; now: string }>; run?: (command: string, args: string[]) => string }): DispatchEffects {
  ...
  return {
    ...
    reconcileReviews: work => reconcileReviews(root, config, { run, work }),
    reconcileProducers: (work, agents) => reconcileProducers(root, config, work, agents, { run }),
    ...
    persist: cursor => writeDispatchCursor(config, cursor),
  };
}
