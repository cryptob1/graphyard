...
import { chmod, lstat, mkdir, readdir, readFile, realpath, rm, statfs, symlink, writeFile } from 'node:fs/promises';
...
export async function dispatchWork(root: string, work: Work, profile: WorkerProfile, agents: HerdrAgent[], run?: (command: string, args: string[]) => string, allWork: Work[] = [work], prepare: (root: string, key: string, profileName: string) => Promise<PreparedWorker> = prepareWorkerLaunch, release: (root: string, key: string, epoch: number, profileName: string) => Promise<void> = releaseWorkerLaunch, agentTimeoutMs = 30_000, observedAt = new Date().toISOString(), options: DispatchOptions = {}) {
  ...
  if (profile.mode === 'existing') {
    ...
    dependencies = prepared.dependencies ?? null;
    ...
    const prompt = workerPrompt(config, work, profile, prepared.epoch, dependencies);
    let pane: string | undefined, tabId: string | undefined;
    try {
      ...
      const sessionHarness = await prepareSessionHarness(root, config, { role: 'worker', kind: profile.kind, profile: profile.name, branch: prepared.branch ?? `graphyard/${work.key.toLowerCase()}-${prepared.epoch}`, credentialFiles: [profile.credentialFile!] });
      ...
      const supervised = [process.execPath, config.cliPath, 'watch', work.key, String(prepared.epoch), '--', profile.kind!, ...launch.args, ...sessionHarness.args].map(shellQuote).join(' ');
  ...
  return { work: work.key, profile: profile.name, principal: profile.principal, agentName: profile.agentName, pane: target.pane_id ?? null, approvals: profile.approvals,
    launch: launchPlan(profile.kind, profile.approvals, profile.agentArgs, profile.environment), ownership: 'worker launcher claimed and is supervising the agent process', harness, dependencies,
