...
export interface SessionHarnessInput { role: SessionRole; kind: string | undefined; cliPath: string; repository: string; baseBranch: string; credentialDirectories: string[]; branch?: string; pr?: number }
export function sessionHarnessPlan(input: SessionHarnessInput): HarnessPlan {
  ...
  if (input.role === 'worker') {
    ...
    if (!branch) throw new Error('A worker harness names the assigned branch it may push');
    allow = [
      ...[`git push origin ${branch}`, `git push -u origin ${branch}`, `git push --set-upstream origin ${branch}`, `git push origin HEAD:${branch}`].map(command => ({ rule: `Bash(${command})`, why: `Push the assigned branch ${branch}; the lease and branch protection bound what it may change.` })),
      { rule: `Bash(${cli}:*)`, why: 'Run the worker protocol under this session\'s own credential: status, heartbeat, sync, blocked, complete.' },
      { rule: 'Bash(gh pr create:*)', why: 'Open the pull request for the assigned branch.' },
      { rule: 'Bash(gh pr view:*)', why: 'Read the pull request for the assigned branch.' },
    ];
    deny = [...secrets, ...noVerdict,
      { rule: 'Bash(git push --force*)', why: 'History published under review is never rewritten.' },
      { rule: 'Bash(git push -f*)', why: 'History published under review is never rewritten.' },
      ...[`git push origin ${input.baseBranch}`, `git push origin HEAD:${input.baseBranch}`].map(command => ({ rule: `Bash(${command})`, why: 'The managed base branch changes only through the guarded merge.' })),
      { rule: `Bash(${cli} evidence:*)`, why: 'Implementation workers never submit trusted evidence.' },
    ];
 ...
 */
export async function prepareSessionHarness(root: string, config: MasterConfig, input: Omit<SessionHarnessInput, 'cliPath' | 'repository' | 'baseBranch' | 'credentialDirectories'> & { profile: string; credentialFiles?: string[] }) {
  const plan = sessionHarnessPlan({ ...input, cliPath: config.cliPath, repository: config.repository, baseBranch: config.baseBranch,
...
export async function dispatchWork(root: string, work: Work, profile: WorkerProfile, agents: HerdrAgent[], run?: (command: string, args: string[]) => string, allWork: Work[] = [work], prepare: (root: string, key: string, profileName: string) => Promise<PreparedWorker> = prepareWorkerLaunch, release: (root: string, key: string, epoch: number, profileName: string) => Promise<void> = releaseWorkerLaunch, agentTimeoutMs = 30_000, observedAt = new Date().toISOString(), options: DispatchOptions = {}) {
  ...
  if (profile.mode === 'existing') {
    ...
    try {
      ...
      const harness = await prepareSessionHarness(root, config, { role: 'worker', kind: profile.kind, profile: profile.name, branch: prepared.branch ?? `graphyard/${work.key.toLowerCase()}-${prepared.epoch}`, credentialFiles: [profile.credentialFile!] });
      ...
      const supervised = [process.execPath, config.cliPath, 'watch', work.key, String(prepared.epoch), '--', profile.kind!, ...launch.args, ...harness.args].map(shellQuote).join(' ');
