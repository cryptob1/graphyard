...
export async function startMaster(root: string, kind: WorkerProfile['kind'], agentArgs: string[], agents: HerdrAgent[], run?: ChildRun) {
  ...
  try {
    ...
    const mergeInstruction = config.autoMerge
      ? `Automatic routine merging is enabled. The loop's merge step performs the guarded merge of a system-driven item when all gates pass; use node ${config.cliPath} master merge only for an item created with "systemDriven": false.`
      : `Automatic merging is disabled, so every merge needs explicit operator approval given by an agent. For a system-driven item the loop requests the merge decision, launches its approver and merges on the approval. For an item created with "systemDriven": false, request it with node ${config.cliPath} master decide GY-N merge REASON and launch the approver; master merge refuses a candidate without an approved merge decision. Never wait on a human for it.`;
