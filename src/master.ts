...
export async function startAgentSession(name: string, kind: string, pane: string, args: string[], text: string, run?: ChildRun, options: PromptDelivery & { timeoutMs?: number; confirm?: 'inline' | 'follow'; retry?: string } = {}) {
  const request = launchRequest(kind, text);
  assertSessionName(name, options.retry);
  try { await herdrJson(['agent', 'start', name, '--kind', kind, '--pane', pane, '--timeout', String(options.timeoutMs ?? agentStartTimeoutMs), '--', ...args, ...request.args], run); }
  catch (error) {
    ...
    const raw = await herdrJson(['agent', 'get', pane], run);
    const agent = raw?.agent ?? raw;
    if (agent?.agent !== kind || !['working', 'idle', 'done'].includes(agent?.agent_status)) throw error;
    await herdrJson(['agent', 'rename', pane, name], run);
  }
  if (request.delivery === 'paste') await deliverPrompt(name, text, run, options);
 ...
 * or a test's stub — and is always awaited: a `herdr agent start` that takes its whole thirty
 * seconds delays only the launch that asked for it, never the cycle's reads beside it (GY-125).
 */
...
export async function waitForHerdrAgent(target: string, run?: ChildRun, timeoutMs = 30_000, onRequest = false) {
  const deadline = Date.now() + timeoutMs;
  let lastError: unknown;
  while (Date.now() < deadline) {
    let agent: any;
    try {
      const raw = await herdrJson(['agent', 'get', target], run);
      agent = raw?.agent ?? raw;
...
export async function startMaster(root: string, kind: WorkerProfile['kind'], agentArgs: string[], agents: HerdrAgent[], run?: ChildRun) {
  ...
  try {
    ...
    ({ delivery } = await startAgentSession(name, kind, created.pane, launch.args, `${prompt} ${reviewInstruction} ${administrationInstruction} ${mergeInstruction}`, run, { confirm: 'follow', retry: masterRetry }));
  } catch (error) {
...
export async function dispatchWork(root: string, work: Work, profile: WorkerProfile, agents: HerdrAgent[], run?: ChildRun, allWork: Work[] = [work], prepare: (root: string, key: string, profileName: string) => Promise<PreparedWorker> = prepareWorkerLaunch, release: (root: string, key: string, epoch: number, profileName: string) => Promise<void> = releaseWorkerLaunch, agentTimeoutMs = 30_000, observedAt = new Date().toISOString(), options: DispatchOptions = {}) {
  ...
  if (profile.mode === 'existing') {
    ...
    const launch = accountLaunch(profile, selected.account, { writable: [await sharedGitDirectory(root)].filter((path): path is string => !!path) });
    launched = launch;
...
async function launchWorker(root: string, config: MasterConfig, work: Work, profile: WorkerProfile, launch: ReturnType<typeof accountLaunch>, run: ChildRun | undefined, prepare: (root: string, key: string, profileName: string) => Promise<PreparedWorker>, release: (root: string, key: string, epoch: number, profileName: string) => Promise<void>, agentTimeoutMs: number, delivery?: PromptDelivery) {
  ...
  try {
    ...
    await herdrRun(['pane', 'run', pane, supervised], run);
    await waitForHerdrAgent(pane, run, agentTimeoutMs, request.delivery === 'request');
    await herdrJson(['agent', 'rename', pane, profile.agentName], run);
    if (request.delivery === 'paste') await deliverPrompt(profile.agentName, prompt, run, delivery);
...
export async function launchApprover(root: string, work: Work, decision: string, kind: NonNullable<WorkerProfile['kind']>, agents: HerdrAgent[], run?: ChildRun) {
  const config = await loadMasterConfig(root);
  ...
  try {
    ...
    pane = created.pane; tabId = created.tab;
    ({ delivery } = await startAgentSession(name, kind, created.pane, launch.args, prompt, run, { retry }));
  } catch (error) {
...
export async function launchEscalationHandler(root: string, config: MasterConfig, context: EscalationContext, kind: NonNullable<WorkerProfile['kind']>, agents: HerdrAgent[], run?: ChildRun) {
  ...
  try {
    ...
    ({ delivery } = await startAgentSession(name, kind, created.pane, launch.args, prompt, run, { retry: escalationRetry }));
  } catch (error) {
    if (pane || tabId) try { await stopCreatedHerdrTab(pane, tabId, run); } catch { /* the launch error below is the report */ }
    throw error;
  }
...
export async function runAutonomyCommand(root: string, config: MasterConfig, id: string, args: string[], deps: AutonomyDependencies) {
  ...
  if (id === 'approver') {
    ...
    return launchApprover(root, work, args[1], kind, await deps.agents());
  }
  if (id === 'principals') {
