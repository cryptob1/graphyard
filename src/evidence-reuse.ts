...
export class EvidenceReuse {
  ...
  async decide(actor: Principal, input: unknown, key: string) {
    ...
    return v.withReceipt(actor, 'reuse', data, key, async (db, now) => {
      ...
      let previous: ValidationCandidate | null = null, evidence: Evidence | undefined, attempt: Attempt | undefined, request: ValidationRequest | undefined, bundle: Bundle | undefined;
      if (!newest) refuse('No sequenced attempt exists for this proof; live verification is required');
        ...
        try { bundle = await v.definition(db, 'bundle', previous.bundle) as Bundle; } catch { refuse('The approved oracle bundle revision the attempt executed is no longer current'); }
      ...
      if (!decision.reasons.length && previous && request && attempt && evidence && bundle) {
        decision.outcome = 'granted';
        // The derived candidate carries the manifest and compatibility signature of the head it
        // stands for, so attribution reads it like any other candidate record of that head.
        const manifest = candidateManifest(build!, previous.environment);
        const { signature, components } = compatibilitySignature({ manifestHash: manifest.hash, sourceSha: w.candidate!.sha, baseSha: w.candidate!.baseSha, policyRevision: w.policyRevision, buildInputsDigest: build!.buildInputsDigest,
          bundle: { digest: bundle.digest, runnerImageDigest: bundle.runnerImageDigest, scenarioHash: bundle.scenarioHash, scenarioRevision: bundle.scenarioRevision }, environment: previous.environment, requiredArtifacts: previous.requiredArtifacts, proof: data.proof });
        const candidate: ValidationCandidate = { workId: w.id, expectedWorkRevision: data.expectedWorkRevision, proof: data.proof, environment: previous.environment, bundle: previous.bundle, buildAttestationId: build!.id, requiredArtifacts: previous.requiredArtifacts, artifactStorage: previous.artifactStorage,
          ...
          manifestHash: manifest.hash, digestHash: manifest.digestHash, signature, signatureComponents: components,
