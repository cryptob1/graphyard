...
export async function closeWork(services: Services, caller: Principal, id: string, body: unknown, key: string) {
  ...
  const ancestry = await closeAncestry(services, data);
  return services.engine.store.transaction(async (db: Db, now: Date) => closeWorkIn(services, db, now, await authenticated(services, db, now, caller), id, data, key, ancestry));
}

type CloseInput = ReturnType<typeof closeSchema.parse>;
type Ancestry = { base: string; contained: boolean } | null;
/** Whether a superseding commit is on the base branch, read from GitHub; asked before the transaction, never inside it. */
export async function closeAncestry(services: Services, data: CloseInput): Promise<Ancestry> {
  const ref = data.ref ?? null;
  if (!(data.kind === 'superseded' && ref && !itemRef.test(ref) && commitRef.test(ref))) return null;
  ...
  return { base, contained: await services.github!.contains(ref, base) };
}

/** `closeWork` inside the caller's transaction, for an actor already authenticated in it (GY-431: a triage closure applies atomically). */
export async function closeWorkIn(services: Services, db: Db, now: Date, actor: Principal, id: string, data: CloseInput, key: string, ancestry: Ancestry) {
  const ref = data.ref ?? null;
