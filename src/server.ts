...
import { Refusal, demand, operatorScopeIncludes, parseReviewerApps, stages, type Principal } from './model.js';
...
async function main() {
  ...
  const timer = setInterval(async () => {
    ...
    try { await validation.expireArtifacts(); await validation.reconcile(); await engine.reconcile(); await delivery.sweep(); await projectFlow(engine.store, { batches: 4 }); if (github) await Promise.all(Array.from({ length: 4 }, () => processJob(engine, github))); }
