...
export function producerPrompt(config: Pick<MasterConfig, 'repository' | 'cliPath'> & { run?: MasterConfig['run'] }, binding: Pick<ProducerBinding, 'key' | 'pr' | 'sha' | 'baseSha' | 'policyRevision' | 'proofs'> & { group: string; requestId?: string; checkout?: string }, profile: Pick<ProducerProfile, 'principal'>,
  ...
  return `You are an independent Graphyard proof producer for ${config.repository}, principal ${profile.principal}. Produce trusted evidence for work item ${binding.key} (pull request #${binding.pr}) at exact head ${binding.sha} against base ${binding.baseSha} under policy revision ${binding.policyRevision}, for the ${binding.group} proof group: ${binding.proofs.join(', ')}. `
    ...
    + `For every unit and integration proof also run the same cases on the base tree, in a second detached worktree of ${binding.baseSha} at ${resolve(checkout.directory, 'base')}, and add "baseline":{"sha":"${binding.baseSha}","result":"pass"|"fail","executed":N,"skipped":N} to its file: a proof that passes on the base as well exercises nothing and is recorded unexercised, and a base where its cases do not exist yet is result fail with executed 0. `
    + `Keep everything this session writes — the install, build output, evidence files — inside ${checkout.directory}; Graphyard removes that directory when the session ends. When every proof of the group is submitted, remove the worktrees with git worktree remove --force ${worktree} (and the base one), print a one-paragraph summary naming each proof and its result, and stop; Graphyard closes this session once it observes the evidence. `
