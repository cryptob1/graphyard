...
/**
 * The report pool (GY-705): each store's last computed flow report per window and filter set,
 * kept with the dataset it was computed from, so `GET /api/analytics/flow` and its drill-downs
 * answer from memory instead of re-reading a week of facts (about 6 s at 250 open items). A
 * report is served for at most `flowReportFreshMs`, and at once recomputed when the flow moved
 * under it: a new flow fact (every step change — a stage, gate, merge or delivery — records one),
 * a new deployment observation (`invalidateFlowReports`) or a new production-watch pass.
 */
...
/**
 * The flow report for `query` from the report pool, computed on a miss. The bounded projection
 * catch-up runs first on every read, so a step event recorded since the pooled report was
 * computed becomes a newer flow fact and the read recomputes; an idle flow is served from the
 * pool. Concurrent misses share one computation, and a failed one is never pooled.
 */
export async function pooledFlowReport(store: Store, query: FlowQuery): Promise<PooledFlowReport> {
  await projectFlow(store, { batches: 3 });
  const lastFact = Number((await store.pool.query('SELECT COALESCE(max(id),0) AS id FROM flow_facts')).rows[0].id);
