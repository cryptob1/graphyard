...
export function daemonEffects(root: string, source: MasterConfig | (() => MasterConfig), deps: {
  ...
  const approver: DaemonEffects['approver'] = async (work, decision) => { const launched = await launchApprover(root, work, decision, undefined, await listHerdrAgents(run), run, {}, handle => deps.mutate(`work/${work.id}/session`, handle)); return { agentName: launched.agentName, pane: launched.pane, session: launched.session, run: launched.run, settled: launched.settled }; };
