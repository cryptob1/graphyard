...
export function daemonEffects(root: string, source: MasterConfig | (() => MasterConfig), deps: {
  ...
  const approver: DaemonEffects['approver'] = async (work, decision) => { const launched = await launchApprover(root, work, decision, current().reviewers[0]?.kind ?? 'claude', await listHerdrAgents(run), run); return { agentName: launched.agentName, pane: launched.pane }; };
  ...
  return {
    ...
    endSession: async (session, resolution) => {
      if (session.pane) await closeHerdrPane(session.pane, run);
