...
const arrayCap = (schema: unknown, field: string) => {
  const array = (schema as { shape: Record<string, any> }).shape[field];
  const inner = array?.def?.innerType ?? array?.unwrap?.() ?? array;
  const maximum = inner?._zod?.bag?.maximum;
  if (typeof maximum !== 'number') throw new Error(`${field} ledger declares no array cap`);
  return maximum;
};
export const reviewLedgerBound = arrayCap(reviewLedgerSchema, 'reviews');
export const producerLedgerBound = arrayCap(producerLedgerSchema, 'producers');
...
const ledgerReading = (records: { state: string; requestId?: string; closedAt?: string; idleSince?: string; requestedAt: string }[] | null, bound: number, input: ResourceInputs, file: string) => {
  ...
  const terminal = records.filter(record => record.state !== 'pending');
  const reapable = terminal.filter(record => !(record.requestId && live.has(record.requestId)) && input.now - settledAt(record) >= ledgerRetentionMs).length;
  return [{ id: '', used: records.length, bound, detail: `${records.length - terminal.length} pending, ${terminal.length} terminal (${reapable} past the ${ledgerRetentionMs / 60_000}-minute retention) in ${file}`, reclaimable: reapable }];
};
const tooBig = (field: string) => new RegExp(`Too big: expected array to have <=\\d+ items[\\s\\S]*"${field}"|"${field}"[\\s\\S]*Too big: expected array to have <=\\d+ items`);
...
export const resourceRegistry: ResourceDefinition[] = [
  {
    ...
    bound: `the reviews array cap of reviewLedgerSchema (${reviewLedgerBound} records); a write past it is refused by the schema`,
    ...
    warnBelow: tenthOf, symptoms: [tooBig('reviews')],
  ...
  },
  {
    ...
    bound: `the producers array cap of producerLedgerSchema (${producerLedgerBound} records); the writer drops the oldest records past it`,
    ...
    remedy: 'graphyard master run --once runs the reclaim pass now; a ledger at its cap drops its oldest records, pending ones included',
    warnBelow: tenthOf, symptoms: [tooBig('producers')],
...
export class ResourceExhaustedError extends Error {
  constructor(readonly reading: ResourceReading) {
    super(`${describeReading(reading)}. ${reading.remedy}`);
  }
}
...
/**
 * A ledger write refused by its own cap names the ledger, even where nothing read it first: the
 * schema's refusal carries the array's field and bound.
 */
export function ledgerExhaustion(error: unknown): ResourceReading | null {
  const text = error instanceof Error ? error.message : String(error);
  for (const [field, resource, bound] of [['reviews', 'review-ledger', reviewLedgerBound], ['producers', 'producer-ledger', producerLedgerBound]] as const) {
    if (!tooBig(field).test(text)) continue;
    ...
    return { id: resource, resource, title: definition.title, unit: definition.unit, used: bound, bound, headroom: 0, warnBelow: definition.warnBelow(bound), state: 'exhausted',
      owner: definition.owner, reclaim: definition.reclaim, remedy: definition.remedy, detail: `a write of a further record was refused by ${definition.ledgerSchema}`, reclaimable: 0 };
  }
  return null;
}
...
export function attributeRefusal(error: unknown, readings: ResourceReading[] = []): Error {
  ...
  const text = error instanceof Error ? error.message : String(error);
  const reading = ledgerExhaustion(error) ?? attributionFor(text, readings);
