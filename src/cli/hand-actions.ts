import { actionClaimMs, claimable, claimLive, producerLaunchStop, settling, waitingToRetry, type ActionRow } from '../model/actions.js';
 ...
 * request (`rowRunning`) means the loop has not stopped, whatever this host's ledger says: the
 * ledgers are host-local, and an executor on another host claims the row without seeing them.
 *
 ...
 * request. Only the row's retirement stops the executor: when the item no longer needs the action,
 * or — for a producer row — once an executor recorded a launch refusal no time gets past
 * (`producerLaunchStop`), which the control plane then stops offering to every executor. A
 * launcher refusal read from this host's ledger alone never counts: an executor on another host
 * claims the same row without that ledger and launches. A review row has no such stop: the executor's `request-review` handler launches through `launchReview`
...
export function producerRecovery(work: Pick<Work, 'autoDispatch' | 'actionQueue'>, proof: string, loop: LoopSessions): string | null {
  ...
  const rows = producerRows(work, request);
  // The durable stop an executor recorded holds for every host; it opens the recovery once the
  // control plane has retired the row it stops, so no executor still holds a claim of it.
  const stop = producerLaunchStop(work, request.id);
  if (stop && !rowRunning(rows)) return `producer request ${request.id} gets no further launch from any executor: ${stop.reason}`;
  return stoppedRequest('producer', request, loop.sessions, loop.failures[request.id], loop.now, rows);
}
