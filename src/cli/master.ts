...
import { agentToken, approvedMerges, assertMasterBinding, autonomySubcommands, continueMergeBatch, decisionInput, runAutonomyCommand, currentMergeCandidates, daemonExecutor, dispatchWork, listHerdrAgents, liveMasterConfig, loadMasterConfig, masterHarness, masterSettingsFromArgs, mergeExecutor, mergeProtocolSkew, producerCommand, readCredentialFile, readWorkerCredential, saveMasterSettings, saveWorkerProfile, snapshotWithClock, startMaster, verifyContainmentDeath, workerProfileSchema, type MasterConfig } from '../master.js';
 ...
 */
export function baseTree(root: string, baseBranch: string, run: typeof execFileSync = execFileSync) {
  try { run('git', ['fetch', '--quiet', 'origin', baseBranch], { cwd: root, stdio: 'ignore', timeout: 30_000 }); } catch { /* resolve against the last fetched base */ }
  for (const [ref, name] of [[`refs/remotes/origin/${baseBranch}`, `origin/${baseBranch}`], [`refs/heads/${baseBranch}`, baseBranch]]) {
    try {
      const listed = run('git', ['ls-tree', '-r', '--name-only', '-z', ref], { cwd: root, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024, stdio: ['ignore', 'pipe', 'ignore'] });
...
export async function derivedIntent(root: string, config: Pick<MasterConfig, 'baseBranch'>, id: 'create' | 'requirements', args: string[],
  ...
  const tree = (deps.tree ?? baseTree)(root, config.baseBranch);
