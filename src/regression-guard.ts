 ...
 * and by `graphyard sync` (local diff against `origin/<base>`).
 */
export type ScopeKind = 'in-scope' | 'new' | 'matches-base' | 'already-absent' | 'reverted' | 'rewritten' | 'deleted' | 'renamed' | 'binary' | 'unverified';
...
export function classifyScope(plannedFiles: string[], files: ScopeFile[]): ScopeFinding[] {
  const findings: ScopeFinding[] = [];
  const add = (path: string, kind: ScopeKind, detail: string) => findings.push({ path, kind, refused: !['in-scope', 'new', 'matches-base', 'already-absent'].includes(kind), detail });
 ...
 */
export function regressionRefusals(work: Pick<Work, 'key' | 'plannedFiles'>, observation: Pick<Observation, 'scopeFiles'>, all: Work[]): string[] {
  ...
  const refused = classifyScope(work.plannedFiles ?? [], observation.scopeFiles).filter(finding => finding.refused);
