...
export class Validation {
  ...
  async createCandidate(actor: Principal, input: unknown, key: string) {
    ...
    return this.withReceipt(actor, 'candidate', data, key, async (db, now) => {
      ...
      const c: ValidationCandidate = { ...data, id: randomUUID(), sourceSha: w.candidate.sha, baseSha: w.candidate.baseSha, policyRevision: w.policyRevision, scenario: s, files: w.observation.scopeFiles?.map(f => ({ path: f.path, sha: f.sha })) ?? null, createdAt: now.toISOString(), createdBy: actor.id };
      await db.query('INSERT INTO validation_candidates(id,document) VALUES($1,$2)', [c.id, JSON.stringify(c)]);
  ...
  async createRequest(actor: Principal, input: unknown, key: string) {
    ...
    return this.withReceipt(actor, 'request', data, key, async (db, now) => {
      ...
      await db.query('INSERT INTO validation_requests(id,document) VALUES($1,$2)', [r.id, JSON.stringify(r)]);
