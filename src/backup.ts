...
import { ledgerOrder, ledgerSeeded, ledgerSequences, ledgerTables, takeBackupLock } from './store.js';
...
export async function createBackup(pool: pg.Pool): Promise<Backup> {
  ...
  try {
    ...
    // The backup lock, never the coordination lock: a backup neither waits behind nor holds up
    // coordination transactions, and only one backup or restore runs at a time.
    await takeBackupLock(db);
