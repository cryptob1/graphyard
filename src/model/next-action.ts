...
const refusalRules: { gate: string | null; match: RegExp; kind: NextActionKind }[] = [
  ...
  // A mechanical proof that failed on the head, or exercises nothing, returns it to its worker (GY-115).
...
export function reviewStandstill(work: Work, all: Work[] = [work], now = new Date()): { kind: NextActionKind; reason: string } | null {
  ...
  switch (need.state) {
    // Mechanical proofs precede review (GY-115): wait on the producers, or return a failed head.
...
export function nextAction(work: Work, all: Work[], now: Date): NextAction | null {
  ...
  if (failing) {
    ...
    if (kind === 'dispatch') {
      // A review refusal is a dispatch only while the head's mechanical proofs have not run.
