...
// Commit and object ids move while a fault stands (a checkout moving further ahead of the loop, GY-531), so they are figures too.
const wording = (text: string) => (text.toLowerCase().replace(/\b(?=[a-f]*\d)[0-9a-f]{7,64}\b/g, '#').replace(/\d+/g, '#').match(/[a-z]+|#/g) ?? []).map(word => word.replace(/s$/, ''))
