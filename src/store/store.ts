...
export class Store {
  pool: pg.Pool;
  /** Lease commands only (pools.ts `leaseCommands`), its failed statements named like the main pool's (GY-447); `background` bounds the tick to half the main pool. */
  ...
  constructor(url: string, options: { max?: number; leaseMax?: number } = {}) {
    ...
    this.leasePool = trackedPool(namedStatements(new pg.Pool({ connectionString: url, max: Math.max(1, Math.floor(options.leaseMax ?? leasePoolConnections)), connectionTimeoutMillis: storeConnectionTimeoutMs, statement_timeout: storeStatementTimeoutMs })));
