...
/** A pool whose failed statements name their SQL (`namedStatements`). */
export const namedPool = (url: string, max: number, connectionTimeoutMillis: number, statementTimeoutMs: number) =>
  namedStatements(new pg.Pool({ connectionString: url, max, connectionTimeoutMillis, statement_timeout: statementTimeoutMs }));
