...
| **Validation runner protocol** | Immutable candidates, approved test bundles, separate runner/collector identities, explicit ACKs, fenced attempts and recovery. |
| **Packaged Playwright runner** | Guided discovery, digest-approved oracle bundles, an isolated executor, and a separately trusted collector that verifies inventory, whole-run target attribution and required artifacts. Rich captures stay disabled until their protection policy ships. |
