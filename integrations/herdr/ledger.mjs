...
const ownership = w => {
  const active = w.lease && Date.parse(w.lease.expiresAt) > Date.now();
...
async function list() {
  ...
  console.log('\n' + rows.map(w => `${clean(w.key).padEnd(8)} ${clean(w.stage).padEnd(11)} ${clean(ownership(w)).padEnd(16)} ${clean(w.title)}\n         ${clean(w.gates.find(g => !g.passed)?.reasons[0] ?? 'All gates passed')}`).join('\n'));
