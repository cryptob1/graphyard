...
test('integration:pi-narrow-roles with pi selected the approver runs headless and its verdict is applied as the approver identity on the approve route, and the server still judges it', async () => {
  ...
  try {
    ...
    const launched = await launchApprover(root, item, decision, undefined, [], herdr(), {}, undefined, { fetcher, filesystem: durable });
    ...
    const refused = await (await launchApprover(root, item, decision, undefined, [], herdr(), {}, undefined, { fetcher, filesystem: durable })).settled!;
    ...
    const stray = await (await launchApprover(root, item, decision, undefined, [], herdr(), {}, undefined, { fetcher, filesystem: durable })).settled!;
