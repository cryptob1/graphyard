...
test('scoped operator-agent credentials are deny-by-default, auditable, rotation-safe, and cannot weaken policy', async () => {
  ...
  const createdResponse = await post('work', { ...workInput, reason: 'Create bounded implementation intent' }, firstToken); const created: any = await createdResponse.json(); assert.equal(createdResponse.status, 200, JSON.stringify(created));
