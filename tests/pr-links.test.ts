...
test('unconfigured or malformed repository identity never produces a link', () => {
  for (const repository of [null, undefined, '', '   ', 'fixtureonly', '/leading', 'trailing/', 'a/b/c', '.', '..', 'a/../b', 'fixture repo', 'fixture/repository?x=1', 'fixture/repo#frag', 'fixture/repo/slash', '-owner/repo', 'owner-/repo', 'ownér/repo', 'fixture/../../evil', 42, {}]) {
...
test('candidate PR and SHA values are validated before any URL is built', () => {
  ...
  for (const badSha of [short, '', 'zzzz', `${sha}0`, ` ${sha}`, `${sha};`, 'javascript:alert(1)', '../../etc/passwd', 123, null, undefined]) assert.equal(candidateCommitUrl(repo, badSha), null, JSON.stringify(badSha));
});
