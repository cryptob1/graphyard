...
import { closureHandle, dispatchEffects, emptyDispatchCursor, launchedSessionHandle, reconcileSessionLiveness, runDispatchTick, type DispatchEffects } from '../src/auto-dispatch.js';
...
before(async () => {
  ...
  database = new EmbeddedPostgres({ databaseDir: await mkdtemp(join(tmpdir(), 'graphyard-session-liveness-')), user: 'graphyard', password: 'testing-only', port, persistent: false, onLog: () => {}, onError: () => {}, postgresFlags: ['-h', '127.0.0.1'] });
...
test('integration:dead-session-reconciled — a session the runtime no longer reports is closed with an outcome naming that it vanished, by a sweep on a bounded interval rather than by anything reporting in', async () => {
  ...
  const effects = dispatcher(async () => [await reload(item)], () => listed);
  const before = await runDispatchTick(fleetConfig, emptyDispatchCursor(fleetConfig), effects);
  ...
  listed.length = 0;
  const young = await runDispatchTick(fleetConfig, emptyDispatchCursor(fleetConfig), effects);
  ...
  // Past the grace, on the next sweep, and within the bound the loop declares: the record closes.
  const startedAt = Date.parse((await handleOf(item, handle.id)).updatedAt);
  const late = new Date(startedAt + sessionVanishGraceMs + 1000);
  assert.ok(late.getTime() - startedAt <= sessionClosureBoundMs, `the closure lands inside the ${sessionClosureBoundMs}ms bound`);
  const swept = await runDispatchTick(fleetConfig, emptyDispatchCursor(fleetConfig), dispatcher(async () => [await reload(item)], () => listed, () => late.getTime()), () => late.getTime());
  ...
  assert.match(closed.outcome!, /^vanished: the claude runtime on host-1 no longer reports pane pane-77/, 'the outcome names that it vanished');
  ...
  const again = await runDispatchTick(fleetConfig, emptyDispatchCursor(fleetConfig), dispatcher(async () => [await reload(item)], () => listed, againAt), againAt);
  ...
  // The rule, directly: an unreadable runtime is never evidence that a session is gone, and a
  // handle with no coordinate to match on has nothing to be reconciled against.
  const live = { pane: 'pane-77', agentName: 'watch-claude', runtime: 'claude' };
  assert.equal(sessionLiveness(live, null), 'unknown');
  assert.equal(sessionLiveness({ pane: null, agentName: null, runtime: 'claude' }, []), 'unreconciled');
  assert.equal(sessionLiveness(live, []), 'vanished');
  assert.equal(sessionLiveness(live, [{ pane_id: 'pane-77', agent_status: 'done' }]), 'ended');
  assert.equal(sessionLiveness(live, [{ pane_id: 'pane-77', agent_status: 'blocked' }]), 'live', 'a session waiting at a prompt still holds its pane');
  ...
  assert.deepEqual(unreadable.filter(closure => closure.cause === 'vanished'), [], 'an unreadable runtime would otherwise close every session on the graph at once');
  // The terminal vocabulary is the runtime's own, and Muse's names are not Claude's.
  assert.ok(runtimeEndedStates('claude').includes('done') && !runtimeEndedStates('claude').includes('idle'));
  ...
  // A runtime that still lists the session but reports it over is the same closure, said differently.
  await engine.execute(coordinator, 'session', item.id, { ...handle, id: 'coordination-2', pane: 'pane-78' }, randomUUID());
  const over = new Date(Date.now() + sessionVanishGraceMs + 1000);
  const ended = reconcileSessionLiveness([await reload(item)], [{ name: 'watch-claude', pane_id: 'pane-78', agent_status: 'done' }], over, { states: runtimeEndedStates });
  assert.deepEqual(ended.map(closure => [closure.id, closure.cause]), [['coordination-2', 'ended']]);
  assert.match(ended[0].outcome, /reports pane pane-78 as done, so the session is over/);
  assert.deepEqual(closureHandle(ended[0]), { id: 'coordination-2', kind: 'coordination', runtime: 'claude', host: 'host-1', subject: handle.subject, state: 'finished', outcome: ended[0].outcome });
});
...
test('integration:slot-released-on-death — the launcher\'s busy check counts reconciled liveness, so a request refused for a busy name launches once the session holding it dies', async () => {
  ...
  const refused = await runDispatchTick(fleetConfig, emptyDispatchCursor(fleetConfig), dispatcher(snapshot, () => listed));
  ...
  const unlisted = await runDispatchTick(fleetConfig, emptyDispatchCursor(fleetConfig), dispatcher(snapshot, () => []));
  ...
  const launched = await runDispatchTick(fleetConfig, emptyDispatchCursor(fleetConfig), dispatcher(snapshot, () => [], () => after.getTime()), () => after.getTime());
  ...
  const busyAgain = await runDispatchTick(fleetConfig, emptyDispatchCursor(fleetConfig), dispatcher(snapshot, () => [{ name: 'review-claude', pane_id: 'pane-review', agent_status: 'working' }], againAt), againAt);
...
test('integration:superseded-session-closed — a review or proof session for a candidate that merged, a superseded head, or an item returned to a worker is closed with the reason, and one item never holds two live sessions for the same role and head', async () => {
  ...
  merged = await reload(merged);
  const mergedClosures = reconcileSessionLiveness([merged], alive(merged), at, { states: runtimeEndedStates });
  ...
  const movedClosures = reconcileSessionLiveness([moved], alive(moved), at, { states: runtimeEndedStates });
  ...
  const returnedClosures = reconcileSessionLiveness([returned], alive(returned), at, { states: runtimeEndedStates });
  ...
  const duplicates = reconcileSessionLiveness([doubled], alive(doubled), at, { states: runtimeEndedStates });
...
test('manual:session-liveness-docs-review — docs/master-agent.md states that the control plane reconciles session liveness, on what interval, and what an operator or master does instead of closing sessions by hand', () => {
  ...
  assert.match(section, new RegExp(`within ${sessionClosureBoundMs / 1000} seconds of its last observed activity`));
  ...
  assert.match(section, /`idle` and `blocked` are deliberately not terminal/);
