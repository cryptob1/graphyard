...
function item(reviews: { state: string; reviewer: string; submittedAt?: string }[], threads = [thread('PRRT_one'), thread('PRRT_two')]): Work {
  ...
  return {
    ...
    observation: { candidate, checks: [], reviews: reviews.map(review => ({ submittedAt: at, ...review, sha: head })), merged: false, mergeSha: null, mergeable: true, protected: true,
...
// Approved longer ago than the loop waits for its own thread resolution (threadResolutionGraceMs).
const approved = { state: 'APPROVED', reviewer: 'graphyard-reviewer[bot]', submittedAt: new Date(Date.parse(at) - threadResolutionGraceMs).toISOString() };
...
test('unit:rework-waits-for-review-and-keys-its-grounds — threads left from an earlier head wait for the current head\'s review, and a verdict on the same head is a separate request', () => {
  ...
  // Just approved: the loop resolves the threads that approval vouches for before asking for rework.
  assert.equal(decide(item([{ state: 'APPROVED', reviewer: 'graphyard-reviewer[bot]' }])), null, 'the approval\'s thread resolution is waited for');

  // Approved, yet threads remain once that resolution had its time: rework keyed on exactly those threads.
  const threads = decide(item([approved]));
  ...
  const fewer = decide(item([approved], [thread('PRRT_two')]));
