...
test('unit:pi-account-key-by-reference — a Pi account names its key file and variable; the headless launch reads the key into that run\'s environment only, and no registry document, log or ledger event holds the key', async () => {
  ...
  try {
    const launched = await launchApprover(root, item('GY-901'), decisionId(1), undefined, [], noHerdr, { registry: registry.client, quota: false, cacheMs: 0 }, undefined, { fetcher: accepted });
...
test('unit:account-smoke-gate — an account is smoke-tested before it is first chosen and after any change to it; a failure makes it ineligible with the runtime\'s own error and nothing launches on it', async () => {
  ...
  const pi = await fakePi();
  try {
    await assert.rejects(launchApprover(root, item('GY-902'), decisionId(2), undefined, [], noHerdr, { registry: registry.client, quota: false, cacheMs: 0 }, undefined, { fetcher: accepted }),
    ...
    await assert.rejects(launchApprover(root, item('GY-902'), decisionId(2), undefined, [], noHerdr, { registry: registry.client, quota: false, cacheMs: 0 }, undefined, { fetcher: accepted }), /failed its smoke test/);
    ...
    const launched = await launchApprover(root, item('GY-902'), decisionId(2), undefined, [], noHerdr, { registry: registry.client, quota: false, cacheMs: 0 }, undefined, { fetcher: accepted });
    ...
    await (await launchApprover(root, item('GY-903'), decisionId(3), undefined, [], noHerdr, { registry: registry.client, quota: false, cacheMs: 0 }, undefined, { fetcher: accepted })).settled;
...
test('unit:account-smoke-gate — two consecutive approver runs on one account that end without a result hold it from the role for an hour, and the loop falls through to the next account', async () => {
  ...
  const launch = async (n: number) => {
    const launched = await launchApprover(root, item(`GY-91${n}`), decisionId(10 + n), undefined, [], noHerdr, { registry: registry.client, quota: false, cacheMs: 0 }, undefined, { fetcher: accepted });
