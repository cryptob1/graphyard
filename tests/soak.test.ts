...
async function simulateDay(options: { hours: number; regression?: 'approvers-left-open' }) {
  ...
  let released = 0, split = false, deploys = 0, cycles = 0, reportedDispatches = 0, broken = false, repaired = false;
  ...
  return { items, final, github, sessions, lost, violations, observed, failures, production, cycles, reportedDispatches, state, dayStart, decided, baseFailure };
}

test('unit:soak-invariants-hold — a simulated day of the real loop: fifteen items delivered and every system invariant holding after every cycle', { timeout: 180_000 }, async () => {
  const began = performance.now();
  const { items, final, github, sessions, lost, violations, observed, failures, production, cycles, reportedDispatches, state, dayStart, decided, baseFailure } = await simulateDay({ hours: Number(process.env.SOAK_HOURS ?? 24) });
