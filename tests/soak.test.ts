...
async function simulateDay(options: { hours: number; regression?: 'approvers-left-open'; docsTrim?: number }) {
  ...
  let released = 0, split = false, deploys = 0, cycles = 0, reportedDispatches = 0, closedTrim = false;
  ...
  return { items, final, github, sessions, lost, violations, observed, failures, production, cycles, reportedDispatches, state, dayStart, docsFilings, docsActions, closedTrim };
}
