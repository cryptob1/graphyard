 ...
 * from hand-off, and reported by the next cycle — and the invariants hold on that detached path.
 * Optimistic merge (GY-500) is on, as by default: disjoint items land past the queue, two that
 * change shared infrastructure queue, and one breaks main after its optimistic merge, so the main
 * guard reads the post-merge runs, reverts it and reopens it. A change to the loop that breaks an
 * invariant fails here, in CI, before it merges; a new behaviour that repeats per cycle, head or
 * item belongs in this world.
 */
