 ...
 * CLEAN at once and one UNSTABLE, a reviewer bot out of quota that fails over, and a `manual:` proof no
 * producer may run, attested by the loop's own request, whose head moves while that request is open. Every one of the
...
async function simulateDay(options: { hours: number; regression?: 'approvers-left-open' }) {
  ...
  return { items, final, github, sessions, lost, violations, observed, failures, production, cycles, state, dayStart, attestations };
}

test('unit:soak-invariants-hold — a simulated day of the real loop: fifteen items delivered and every system invariant holding after every cycle', { timeout: 180_000 }, async () => {
  const began = performance.now();
  const { items, final, github, sessions, lost, violations, observed, failures, production, cycles, dayStart, attestations } = await simulateDay({ hours: Number(process.env.SOAK_HOURS ?? 24) });
