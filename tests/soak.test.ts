...
async function simulateDay(options: { hours: number; regression?: 'approvers-left-open' }) {
  ...
  return { items, final, github, sessions, lost, violations, observed, failures, production, cycles, state, dayStart, producerRuns, abandoned, spentHead, actionKeys };
}

test('unit:soak-invariants-hold — a simulated day of the real loop: fifteen items delivered and every system invariant holding after every cycle', { timeout: 180_000 }, async () => {
  const began = performance.now();
  const { items, final, github, sessions, lost, violations, observed, failures, production, cycles, dayStart, producerRuns, abandoned, spentHead, actionKeys } = await simulateDay({ hours: Number(process.env.SOAK_HOURS ?? 24) });
