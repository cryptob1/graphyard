...
test('unit:cycle-latency-budget — with 170 open items and 1,100 worktrees at the measured latencies, a cycle fits 30 s and master status 20 s, and independent launches run at once, bounded by capacity', async () => {
  ...
  try {
    ...
    // The items opt out of research before build (GY-259), which would otherwise hold them for a research run first.
    ...
    const ready = Array.from({ length: 6 }, (_, index) => ({ ...item(500 + index, new Date().toISOString()), stage: 'build', ready: true, blocker: null, research: false, plannedFiles: [`src/file-${index}.ts`] }) as unknown as Work);
