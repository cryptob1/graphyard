...
import { exhaustionAttention, githubBudgetAttention, pauseAttention, webhookAttention } from '../src/cli/status-attention.js';
...
test('unit:unchanged-candidate-observation-cost — observing the same unchanged candidate twice costs no uncached request the second time, and the mean cost per observation is reported', async t => {
  ...
  assert.equal(second.requests, first.requests, 'the same reads are made');
  ...
  assert.equal(budget.observations.meanRequests, first.requests);
...
/** One signed webhook delivery, as GitHub sends it. */
async function webhook() {
  process.env.GITHUB_WEBHOOK_SECRET ??= 'webhook-secret-for-the-test';
  const raw = JSON.stringify({ action: 'synchronize', repository: { full_name: REPOSITORY } });
...
const prove = async (work: Work) => engine.execute(producer, 'evidence', work.id, { proof: 'unit:budget', sha: work.candidate!.sha, baseSha: work.candidate!.baseSha, policyRevision: 1, result: 'pass', executed: 3, skipped: 0 }, randomUUID());
...
test('integration:observation-cadence-by-state — a merge-gate candidate is observed every 20 seconds, an item whose next action is a dispatch no more often than every five minutes, everything else between, from the item state alone; and a webhook wake observes the slow one at once', async t => {
  ...
  assert.ok(activeMs > 20_000 && activeMs < 300_000, `everything else sits between (${activeMs})`);
