...
test('manual:launch-authorization-onboarding-review — repository setup writes the launch authorization into the generated instructions, a Claude Code role session carries it on its command line, and docs/onboarding.md states what is generated and why', async () => {
  ...
  const guide = await readFile(join(repositoryRoot, 'docs/executors.md'), 'utf8');
  for (const fragment of ['### The request is the session\'s first message', 'run.acknowledgementSeconds', 'awaiting acknowledgement', 'never started', 'counts.dispatchAwaiting', 'retry.neverStarted', '--append-system-prompt']) assert.ok(guide.includes(fragment), `docs/executors.md states ${fragment}`);
