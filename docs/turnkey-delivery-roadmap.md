...
- Upgrade and restore exercises preserve assignments, event history, scenario revisions and pending requests. — `tests/backup-restore.test.ts` (an assignment under lease with its workspace, append-only history, two scenario revisions, a validation request mid-collection with a private artifact, restored into a fresh database, migrated again, and continued under the same epoch), the same round trip through the shipped image in `scripts/verify-image-release.mjs`, and the chart's upgrade-then-restore drill in `deploy/helm/exercise.sh`.
