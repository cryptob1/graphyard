...
An escalation is not a blocker: it refuses the merge gate until a human operator clears it with `graphyard resolve GY-N TRIGGER "audit reason"`. The request must name the standing trigger, the reason is recorded in the event ledger, and no AI principal — lead, worker, producer, or scoped operator agent — can resolve one. See [slice-lead delegation](delegation.md#an-unresolved-escalation-refuses-delivery).
