...
- **Main ahead of production**: a deployment incident: [merged but not deployed](operations-reference.md#merged-but-not-deployed). An up-to-date release starts without taking coordination locks; a migrating release fails fast rather than outlasting the health check.
