...
- Authenticated `/api/status` should show no persistent integration errors, no `delegationLimits.attention`, and no open `production.incidents`; `/healthz` should report the `commit` you last deployed.
