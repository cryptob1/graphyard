...
- **Main ahead of production**: a [deployment incident](operations-reference.md#merged-but-not-deployed). An up-to-date release starts without taking coordination locks; a migrating release fails fast within the health check, and under live traffic it migrates safely: it touches only the tables whose DDL changed since it recorded a digest per table (unchanged tables are skipped without any lock), locks each changed table whose trigger it rebuilds before touching it, and retries a deadlock or expired lock wait with backoff inside one 30-second lock budget, releasing every failed attempt's locks at once; each attempt waits at most 3 seconds for a lock, so live writes never queue behind it longer; migrations and backups lock separately. The first `work_index` rebuild briefly locks `work_items` (SHARE ROW EXCLUSIVE).
...
```sh
graphyard rework GY-N --previous-worker-stopped "reason"                     # worker stopped first
