...
Before a merge can complete work, Graphyard must have recorded an authorization for the same head, base, and policy. A routine automatic merge additionally acquires a short-lived, single-use execution authority in a transaction. While it is active, requirement, evidence, validation, reconciliation, and non-matching observation mutations are refused. This freezes the control-plane decision while the external GitHub merge runs without holding a database transaction open. A failed client cancels the authority; an abandoned authority expires. The engine consults ledger snapshots at the reported merge time, so an outage or later failure does not erase historical authorization. The delivery record references the authorization revision and observed merge SHA. Later differing checks remain visible as a follow-up warning.

Evidence received after an earlier merge cannot retroactively invent approval. For direct external merges without an active execution authority, authorization must predate the earliest possible merge instant and changes within the provider timestamp's precision window are considered conservatively. GitHub's whole-second timestamps can therefore cause same-second direct authorizations to be refused. A matching active single-use execution establishes the causal order for master merges without relying on provider timestamp precision. Artifact verification remains necessary to prove that the merged artifact itself was tested.
