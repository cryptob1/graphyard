...
`GET /api/board` (`src/model/board.ts`) serves these groups. Items carry `group`, `stage`, `owner`, `actor` (who acts next), `command` (or null), `since` and `overdue` (past `overdueAfterMs`); `master status` lists the master's as `board.owed`.
...
Rows show **Build, Validate, Test, Review, Prove, Merge, Deploy**. A merged item reads *Merged*, then *Live* in production. Moving and Blocked rows past thirty minutes read `1h 12m overdue`.

## An item page

Below the summary: **What is left** (unmet requirements, who clears each), **Requirements** (✓ or ○ per criterion), **Pull request** and **Activity**. **Technical details** holds gates, sessions, evidence and overlaps.
