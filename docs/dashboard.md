...
| Entry | What it holds |
| --- | --- |
| **Work** | The home page: open items, grouped by what needs attention; **Needs you**, everything only you may answer; and **Workers**, every agent session across every item, as tabs. |
...
- **Copy local** — run on the host that launched the session: `herdr agent attach w1V:pJD`. Herdr's direct terminal attach opens that one pane's terminal in yours (detach with `ctrl+b q`); `agent attach` takes the pane ID as its target, and a pane ID is already workspace-qualified. The command is built from the pane the handle records. Launchers record it as `herdr pane attach w1V:pJD --workspace w1V`, which the installed Herdr (0.9.1) does not have — it exits 2 on `pane attach` — so the tab reads the recorded pane out of that command rather than copying it as it stands. A recorded command that is not Herdr's is copied exactly as recorded.
- **Copy remote** — run from another machine, through Herdr's remote machinery, built from the handle's recorded host. The installed `herdr --help` documents `herdr --machine <label-or-id> <command>` ("Run an API command on a saved SSH machine") and `herdr --remote <ssh-target>` ("Attach through SSH to a remote Herdr server"), and `herdr --skill` adds that interactive attachment is not forwarded by `--machine`, so `herdr --machine vishrog agent attach …` cannot work. The remote form therefore uses both: an API command on the saved machine focuses the pane, and a remote attach opens that server's UI, which shows its focused pane: `herdr --machine vishrog agent focus w1V:pJD && herdr --remote vishrog`. It needs the host name the handle records to be both a saved Herdr machine label (`herdr machine list` shows them; `herdr machine add` saves one) and an SSH target your `ssh` resolves, such as a `Host vishrog` entry. A recorded command that is not Herdr's has no remote form, and the row says so.
...
The derivation — stale, the two forms, the order and the per-principal summary — is `workersView` in `web/workers-view.ts`, beside the page rather than in the control plane's model, which never names a runtime; and `tests/workers-tab.test.ts` renders the page over a fixture and asserts each of them.
