...
- **With GitHub configured:** both become links built only from the configured repository identity, the observed pull-request number and hexadecimal SHA, so candidate-authored values can never select a URL
