...
- **A truncated scan** reads facts in `(observed_at, id)` order, so it loses the end of the window. `window.covered` and `coverage.covered` state the interval: the requested `from` and `to`, the instant reached (`toCovered`), `ms` against `windowMs` and `fraction`, the `uncovered` interval, and `remainingFacts` there (`remainingCapped` marks a floor). `window.covered.statement` says it in a sentence, `window.truncated` is the flag to branch on, and an export's metadata rows carry `windowCovered`, `windowCoveredFraction`, `windowTruncated` and `windowCoverage`. Narrow the window, type or slice to cover the rest
...
- **Loading:** a read is in flight; figures still on screen are labelled an earlier observation, and the previous coverage state is never carried over
...
- **Partial:** the scan bound was reached; the report names the interval it covered in `window.covered`
