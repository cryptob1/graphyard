...
Spelled in full, the forms are `master registry propose`, `master registry runtime set`, `master registry model set`, `master registry account set`, `master registry account quota`, `master registry role set`, `master registry session end` and `master registry history`. Every command takes `--reason R`, a `set` names only what changes, and a contract carrying a secret is refused. Routes: `POST /api/agent-registry/(runtimes|models|accounts|roles|apply|select)[/ID/(remove|quota)]` and `POST /api/agent-registry/sessions/[0-9a-f-]{36}/end`; reads `GET /api/agent-registry`, `GET /api/agent-registry/document` and `GET /api/agent-registry/history` (`limit`, default `?limit=100`).
...
- **Checked before every launch:** the first account logged in and holding quota — no unreset usage window at or above `run.quotaCeilingPercent` (default 95). Claude's 5-hour and 7-day windows come from the provider's usage endpoint and Codex's from its newest session's rate limits; OpenCode and Cursor expose none, so theirs is `unknown` and never blocks a launch
- **Failover:** an account failing either check is skipped with its reason and the launch fails over to the next; a worker profile with none claims nothing and reads unavailable (`workers[].credential`), review falls through `run.reviewerProfile`, and a producer request goes to the next independent producer
- **Reported** under `dispatch.accounts`: each environment as the last check saw it, with recent skipped launches by role, profile, item and reason, beside the coordinator credential (`*.environments.json`, mode 0600), never holding a provider token
