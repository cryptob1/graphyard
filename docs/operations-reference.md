...
Observation claims `GRAPHYARD_OBSERVATION_CONCURRENCY` jobs at once (default 4, at most half the pool), each `SKIP LOCKED`: queue head and `max(2, batchSize, parallelTips)` band first, then never-observed submissions, any job due over five minutes, review/rework waits, then `available_at`; `master status` raises `github` once the head's observation passes two minutes. Watch `observationThroughput` lag, budget.
