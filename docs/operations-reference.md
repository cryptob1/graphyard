...
`node scripts/rework-causes.mjs` classifies each rework round over the last 100 deliveries by its ledger reason, names each cause's share, and links the three largest to open fix items. `master status` carries the split under `speed.reworkRounds.ownChange`: rework median excluding out-of-item causes. 2026-09-26: 55% own-change findings, 33% base conflicts; raw median 2 falls to 0 without them.
