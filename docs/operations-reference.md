...
| `idle` | The next action is a dispatch, a rework or an escalation | 5 minutes | Nothing on GitHub can move it |

A webhook delivery wakes every job at once whatever its band (`woken` on the claimed job), so a slow-cadence item is observed immediately when GitHub says something changed. Observation is webhook-first and polling is the safety net: for a candidate whose head, base tip, check state and review state are unchanged since its last observation the steady-state poll interval is never under two minutes, and the fleet-wide steady-state spend is bounded to a documented share of the hourly limit — **at most 40%** (`steadyStateShare`) — by stretching the steady interval so that every open candidate polling at it for an hour stays inside that share, counting every request (a free 304 included) and using the measured mean cost per observation (ten requests until one is measured). `githubBudget.steadyState` reports the open candidates, the mean cost and the interval in force. The merge path, webhook wakes and the master session's own reads always have the remaining headroom.
