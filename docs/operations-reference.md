...
The reconciliation tick reads only the items that can change — everything except settled deliveries, whose summaries come from the work index — and reads each once per pass, however many batches it takes; a later batch re-reads only an item whose row moved since, found by the live rows' versions, so pass cost grows with the live items, not with items × batches. Each batch locks only its own items' rows (`SELECT … FOR UPDATE`), so a heartbeat or a request on any other item commits while it runs. A batch that wrote commits only under the coordination lock, taken without waiting, and only if no other item moved while it evaluated; otherwise it rolls back and runs again, and after three such retries takes the lock from its start. A tick slower than 5 s logs a warning naming its item count and duration.
