...
2. Graphyard decides whether the observed target **contains the intended change**, from trusted records only: a builder's attestation for this item and environment whose artifacts are the observed manifest and whose source is the current candidate, or a release of this environment whose manifest is the observed one and whose source is the candidate. Another environment's records say nothing here.
