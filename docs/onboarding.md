...
`master status` reads the answer from systemd every time it runs. A supervisor that is missing, disabled, stopped, or unreadable becomes an attention item naming the exact command that fixes it (`systemctl --user enable --now graphyard-master.service` for a disabled unit, `systemctl --user start graphyard-master.service` for a stopped one, and `master init` on this host when no unit is installed), and the same facts appear under `setup.supervisor`.
