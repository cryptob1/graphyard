...
Profiles default to `"approvals": "auto"` so sessions never block on a permission prompt (the trade-off: an unattended session); `"prompt"` is the opt out ([approval modes](master-agent-sessions.md#approval-modes)).
