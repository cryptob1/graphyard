...
2. **GitHub check publication cannot supply atomic merge authorization.** A successful check may remain during an outage or after an evidence revocation. GitHub check runs are commit-scoped: another PR using the same head can inherit the result. A restricted merge broker must bind authorization to PR/head/base/policy and control alternative merge identities. This is GY-4. The new publication guards reduce races but do not solve this boundary.
