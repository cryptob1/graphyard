...
`graphyard init --scan --apply` derives `GRAPHYARD_GENERATED_FILES` from the managed repository's generated-file manifest when one exists and reports the line under `generatedFiles` beside the principals instruction, to be set beside `GRAPHYARD_PRINCIPALS` with the capacity lines. The Railway adapters stage the capacity variables beside `GRAPHYARD_PRINCIPALS` but not this one, so a deployment whose repository declares generated pages needs the line set by hand; `graphyard master status` reports the drift between the deployed value and the manifest with the exact command that fixes it. The manifest JSON (`node scripts/check-docs.mjs --manifest`) is the declaration of record and wins whenever it parses, and a well-formed `--list` line serves a script that predates it; a script that fails, or prints prose or a value the server would not parse, is refused with a clear message, and a repository without the manifest script leaves the variable unset.
