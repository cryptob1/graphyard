...
Re-running the installer after the principal set changes reports every deployed value that no longer covers the principals before it sets the corrected ones. `scripts/provision-railway.mjs` reads the deployed values from the running server when `GRAPHYARD_URL` is set and prints `Drift: …` for each.
