...
Every installer adapter also derives `GRAPHYARD_GENERATED_FILES` from the managed repository's generated-file manifest when one exists and sets it beside `GRAPHYARD_PRINCIPALS`: the value `node scripts/check-docs.mjs --list` prints for the repository's manifest script (falling back to the manifest JSON), refused with a clear message when the script fails or prints a value the server would not parse, and left unset for a repository without the script. `graphyard init --scan --apply` reports the line under `generatedFiles` beside the principals instruction.
