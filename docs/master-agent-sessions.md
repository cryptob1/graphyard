...
On a runtime's own limit notice (never agent text) the loop commits unpushed `WIP:`, records `capacity.exhausted` (not `lease-loss`) and relaunches on the next account or waits for a reset.
