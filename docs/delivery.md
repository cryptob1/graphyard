...
Graphyard records which release each environment should run, verified only by service-scoped observers; rollback is in [recovery](recovery.md#rollback).
...
Approve with `POST /api/delivery/approve`, then select (one concurrent selection per generation wins):
