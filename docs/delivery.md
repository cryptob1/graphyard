...
| Rollback request, operation, resolution | Operator or promoter; `producer` with a `rollback` registration and a current lease; operator | The [D4 rollback workflow](recovery.md#rollback): a selection of a previously verified release plus one fenced provider operation |
...
- An authoritative observation is also what validation requests bound to this environment are attributed against: in the same transaction, a request whose candidate manifest the observation contradicts is superseded and [re-anchored](attribution.md#re-anchoring), and an accepted pass the observation contradicts inside its execution window stops counting. A non-authoritative observation touches neither.
