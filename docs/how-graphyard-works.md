...
Graphyard decides whether work may move forward; runtimes such as Herdr run the sessions that do it ([glossary](glossary.md)).
...
A card stops at its first refusing gate and says what is missing; nothing can set a stage directly.
...
Text equivalent: in bootstrap the human operator supervises one worker while gates are activated; normally the master dispatches to many workers, each with its own credential and worktree; reviewers and producers judge candidates under the same gates.
