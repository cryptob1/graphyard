...
After final verification and clock-skew waiting, `merge-commit` transactionally rechecks the authorization immediately before the GitHub mutation. It is the serialization point: a withdrawal that wins the work-item row lock cancels provider authority, while a commit that wins marks the provider mutation irrevocably in flight and later withdrawal refuses. Delivery requires that committed execution and its exact candidate. No database transaction spans the network call; an unknown provider outcome retains the committed execution for observation instead of retrying or pretending that a later withdrawal recalled it.
