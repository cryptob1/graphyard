...
An execution authority freezes competing mutations for up to two minutes, which would otherwise make the window *wider* for the one event that most needs to interrupt a merge: an operator or trusted producer withdrawing the evidence that authorized it. [Revocation](protocol.md#revocation) is therefore the single command an active execution cannot defer. It cancels the execution in the same transaction that withdraws the evidence, using the same cancellation ledger the broker and delivery attribution already read. Afterwards `merge-acquire`, `merge-verify` and `merge-cancel` all refuse the cancelled execution, an idempotent replay of the original acquire refuses, concurrent and retried acquisitions refuse because no current authorization exists, and a merge that GitHub nonetheless reports is recorded as a permanent violation rather than delivery.

The residual window is the interval between the final verification committing and GitHub completing the provider merge call — under two seconds in the supported flow, and bounded by clock offsets the verification records. A revocation inside that window cannot recall the provider call; Graphyard refuses the delivery and surfaces the violation. Revocations ordered before the reported merge instant refuse it; revocations after it leave a legitimate delivery intact.
