...
GitHub dismisses stale reviews on any push to the branch, Graphyard's own mechanical tip publication included, and a native review requirement then demands an approval after that push. The control plane's gate is already satisfied by the carried record; the provider's requirement is met by the master, which — immediately before acquiring merge authority — re-posts the carried approval bound to `H'` through the reviewer App that gave it, and only that identity, never through the control-plane App (`repostCarriedApproval`). A human approval cannot be re-posted and is reported as such; a reviewer that has since requested changes is never overridden. The re-post is idempotent: an approval already standing on the tip is left alone.

Never weakened: a tip with any human or worker commit, any conflict resolution, or any change to reviewed files still requires a fresh independent approval and fresh evidence; the reviewer identity and the exact-commit binding stay the rule. The carry only defines when two commits are provably the same reviewed content.
