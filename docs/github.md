...
2. **exercise** runs one job per planned proof, named after the proof, in parallel and without any secret. `prepare-acceptance.mjs` refuses a head that no longer matches the pull request and fetches the candidate at the planned head merged with the planned base. An integration contract builds the candidate image and runs `run-acceptance.mjs` exactly as a dispatch does; a `unit` contract (`scripts/unit-contract.mjs`) copies the protected test file over the candidate's copy and runs its registered titles against the candidate's source with `run-unit-acceptance.mjs`, judging the fixed inventory from the TAP stream. Because a unit inventory shares a process with the code it tests, the lane certifies that the protected inventory passed, not that the candidate could not have interfered with its own test run; integration contracts judge from outside the container. Each job uploads an attempt- and proof-scoped report artifact.
