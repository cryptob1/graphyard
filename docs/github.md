...
- One tip needs no merge: when `H` already contains its new predicted base — the entry ahead was ejected and the base branch did not move — Graphyard moves the branch back to `H` and republishes it as the tip, producing no commit. The tip is then `H` itself, and the rule above applies with nothing to check about parents or author: the files that changed between the replaced tip's bound base and the predicted base are listed from GitHub (`queue.speculation.baseChanges`), the approval carries when none of them is a reviewed file, and each proof by its scope. A list GitHub could not report completely, or an unvalidated predecessor, still refuses every binding. What GitHub said when it dismissed the approval on the earlier publication plays no part: the record carries it.
