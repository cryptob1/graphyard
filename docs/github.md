...
The gate requires `GITHUB_CI_APP_IDS` and protection-required CI checks, current-head approval, trusted passing evidence, a mergeable non-draft PR and the queue head. Unknown mergeability (`null`) is re-read thrice in 10 s, then refused as computing; queued tips decide.
