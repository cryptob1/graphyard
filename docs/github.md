...
The gate also needs `GITHUB_CI_APP_IDS` CI checks, head approval, trusted evidence (executed > 0, skipped 0), a mergeable PR and the queue head.

## Merge queue

A candidate enters once its gates pass. Its speculative tip is pushed onto the candidate branch and `refs/graphyard/queue/KEY`; every check, review and proof binds it. A failed check, requested changes, a revoked proof, a conflict or rework ejects it to re-enter at the back once repaired. The App passes the check for an authorized head and merge group, then asks GitHub to merge; withdrawal dequeues; refusals (`merge.enqueue.refused`) show in status.
