...
The base branch's head and tree are read from `refs/heads/<base>` on every observation. A pull request's own `base.sha` is GitHub's cached view of that ref, refreshed only when GitHub recomputes the pull request, and Graphyard never binds anything to it: predictions decided against the cached value were the root cause of approvals dismissed for "a merge base that changed" when nothing had. The observation records the real tip, its tree, and whether the head contains that tip. A review is requested only for a head that does — `master review` and the `codex` and `agent` review-provider dispatchers all refuse otherwise, and `diagnose` reports `base-behind` — because a review of a head behind the branch is reviewed against a diff GitHub will later recompute. That is a wait of seconds, not a round of work: the [base refresh](#base-refresh-for-in-flight-candidates) below brings the head onto the moved tip on its own.

Graphyard then merges the predicted base into the candidate branch, publishes the resulting commit under its own `refs/graphyard/queue/KEY` ref, and rebinds the candidate to it. That commit is the **speculative tip**. Required checks, the review, and every acceptance proof must then reference that exact commit; bindings made against the replaced head are refused, not reused — except where Graphyard can prove the tip is the same reviewed content, which is the [binding carry](#binding-carry-across-a-graphyard-authored-tip) below. The base recorded for the candidate is the predicted base, not wherever the base branch happens to point.
...
The queue owns the last hop, but a merge moves the base branch under every candidate that has not reached it yet — one in review, one running CI, one collecting proofs. Those candidates used to pay for the merge: the base they were bound to was no longer the branch head, so their approvals and proofs stopped applying, no review could be requested for a head that no longer contained the tip, and the only way forward was a full rework round each — worker sync, push, fresh review, fresh proofs — for what is almost always a clean fast-forward. With several items in flight every merge re-queued all of them, so throughput fell as parallelism rose. Graphyard absorbs the move itself now, in two parts.

**The bound base is held.** While a candidate's head does not contain the branch tip, and the branch still contains the base that head was observed against, the candidate stays bound to that base (`candidate.baseSha`) rather than to the live head. Somebody else's merge did not change the tree this candidate was reviewed and proved on, so the bindings to it stand. The observation still records the real `baseTip` and `baseTipContained: false`, so nothing is hidden. The hold is given up the moment it stops being true: a head that contains the tip binds to the tip, a rewound branch that no longer contains the held commit rebinds to the live head, and a refresh that reports a conflict (below) rebinds too.

**The head is republished on the new tip.** The reconciliation job merges the base branch into the candidate's own pull-request branch — the same conflict-free provider merge the queue uses for a speculative tip, needing the same Contents: write, and no ref of its own since the pull-request branch is the record. It then decides binding carry from GitHub's account of the commit it produced, by the same rule as [the carry below](#binding-carry-across-a-graphyard-authored-tip), with the base branch itself as the predecessor: the approval carries when the base changed none of the reviewed files, and each proof carries when its declared `scopeFiles` are disjoint from the change. The decision is recorded on `baseRefresh` with the reason per binding, in the ledger as `base.refreshed` and `base.carry`, and reported by `master status` (`base`) and `diagnose` (`base-refresh-carried`, `base-refresh-required`). A candidate that is only waiting for this is never an attention item: nobody is being asked for anything, and `master run` reports the refresh as a `refresh` action rather than counting nothing for the cycle.

One attempt is made per head, base tip and policy revision, and a queued candidate is left alone — the queue refreshes its entries through its own speculative tip. Required CI checks are not carried, here or anywhere: the republished head is a different tree, so the checks run on it, which is the point of testing the merge result. An item waiting on review, on CI or on proofs therefore stays where it is; one that had already passed CI waits for it again on the new commit, and for nothing else.

**A conflict is the one case the worker still owns.** When GitHub refuses the merge with a conflict, nothing is written: no commit, no ref, no carried binding. The conflict is recorded with the head and the tip it could not reach, the build gate names it, the item returns to `build` for a fresh attempt, and the held base is given up so the approval and the proofs are invalidated exactly as they always were. `master status` raises it immediately, addressed to the master with the rework decision as the next command. Resolving a conflict is new content nobody reviewed or proved, so nothing may carry across it.

### Tree-identical base advances

When the entry ahead merges with `merge_method=merge`, the base branch becomes a new merge commit whose sha differs from the tip the follower was validated on but whose tree is identical to it. The follower's predicted base therefore changes sha without changing content. Graphyard compares trees, not shas, for that case: the published tip, the candidate binding, the review binding and every evidence binding are kept, nothing is republished, and the pull request branch is not moved. The record notes the advance as `queue.speculation.carriedBase` (the new sha and the shared tree) and the ledger gains a `queue.base-carried` event naming the tip, the bound base, the tree and the base tip it was carried to, so an audit shows why the binding survived. The merge broker performs the same tree check immediately before the provider call. A base advance that changes the tree is stale, as before: the head republishes and the entries behind it are re-based in turn.

### Binding carry across a Graphyard-authored tip

Publishing a tip for an entry that is not already on its predicted base replaces the approved, proven head `H` with a new merge commit `H'`. Graphyard records how it produced that commit — GitHub's account of its parents and author, and the files that changed between `H`'s bound base and the predicted base — and decides once, when it binds the tip, which of `H`'s bindings carry to `H'`:

- The tip must be a two-parent merge whose parents are exactly `H` and the predicted base (the validated tip of the entry ahead, or the observed base tip), authored by the control-plane App, produced by Graphyard's own conflict-free provider merge, over a predecessor that is itself fully validated on that tip. A tip that fails any of these — a commit not authored by the App, a merge with any conflict resolved, an unvalidated predecessor, or a change list GitHub could not report completely — carries nothing: every binding is re-required and the reason is recorded.
- The approval of `H` carries to `H'` when the predecessor changed none of the files `H` changed (the files the reviewer read). It is recorded as carried, with the reviewer identity, the original sha and the review it came from, and the review gate accepts it for `H'` under the same exact-commit rule. A predecessor that touched any reviewed file requires a fresh independent approval.
- Trusted evidence bound to `H` carries per proof: a record carries when it declares `scopeFiles` (the paths the proof depends on, in the `plannedFiles` scope syntax, submitted with the evidence) and that scope is disjoint from what the predecessor changed. A proof whose scope intersects the change, or whose evidence declares no scope, is re-required. A predecessor that changed nothing relative to the bound base carries every proof.
- Required CI checks run on `H'` as before; nothing about the test gate is carried.
- One tip needs no merge: when `H` already contains its new predicted base — the entry ahead was ejected and the base branch did not move — Graphyard moves the branch back to `H` and republishes it as the tip, producing no commit. The tip is then `H` itself, and the rule above applies with nothing to check about parents or author (`decideIdentityCarry` in `merge-queue.ts`): the files that changed between the replaced tip's bound base and the predicted base are listed from GitHub (`queue.speculation.baseChanges`), the approval carries when none of them is a reviewed file, and each proof by its scope. A list GitHub could not report completely, or an unvalidated predecessor, still refuses every binding. What GitHub said when it dismissed the approval on the earlier publication plays no part: the record carries it. Nothing is requested for the replaced head between the publication and the observation of the tip.

The decision lives on the record as `queue.speculation.carry` and in the ledger as a `queue.carry` event, one entry per binding with `carried` or the required reason; the `queue.predicted` event summarises it. A carried approval is a standing judgement, not a copy: it applies only while the candidate is exactly that tip under the same policy revision and, for agent review, while the policy still dispatches to the same reviewer App. A carried proof is the original evidence record, so revoking it withdraws the carried binding, ejects the tip from the queue, and counts as an adverse conclusion exactly as a revocation on the tip itself would.

GitHub dismisses stale reviews on any push to the branch, Graphyard's own mechanical tip publication included, and a native review requirement then demands an approval after that push. The control plane's gate is already satisfied by the carried record; the provider's requirement is met by the master, which — immediately before acquiring merge authority — re-posts the carried approval bound to `H'` through the reviewer App that gave it, and only that identity, never through the control-plane App (`repostCarriedApproval`). A human reviewer's approval cannot be re-posted and is reported as such; a reviewer that has since requested changes is never overridden. The re-post is idempotent: an approval already standing on the tip is left alone.

Never weakened: a tip with any commit the control-plane App did not author (a human operator's or a worker's), any conflict resolution, or any change to reviewed files still requires a fresh independent approval and fresh evidence; the reviewer identity and the exact-commit binding stay the rule. The carry only defines when two commits are provably the same reviewed content.

When the base branch advances outside the queue, only the head re-validates against it. Entries behind it have no predicted base until the head settles; Graphyard then re-bases each of them in turn onto the new chain. The worker is not asked to rebase and no rework is requested: the entry keeps its assignment, workspace, and position, and its stale bindings are refused until the new tip is validated — or carried, under the rule above.
...
Queue position, predicted base, predicted tip, and per-entry wait time appear in `graphyard master status` and on the dashboard, and each queued entry carries a `binding` there: whether its approval and each required proof bind the tip exactly, were carried, or are required afresh, with the recorded reason, plus the tree-identical base advance when one was carried. `graphyard diagnose GY-N` reports the same as `queue-binding-carried`, `queue-binding-required` and `queue-base-carried` diagnostics. See [master-agent operating mode](master-agent.md#merge-queue).
...
The same protected workflow also runs on every push to a `graphyard/*` branch with an open pull request into the base branch, through `pull_request_target`. GitHub takes the workflow file and the harness checkout from the default branch for that event, sets `GITHUB_SHA` to its tip, and evaluates the reporting environment's branch policy against the default branch, so a candidate push runs `main`'s definition with `main`'s secrets while the candidate itself is only ever fetched into an isolated build context. The run has three jobs:

1. **plan** (`scripts/enumerate-ci-proofs.mjs`, in the `graphyard-reporting` environment) reads the control plane with the CI producer credential, finds the item the pull request is the candidate of, waits up to four minutes for Graphyard to observe the pushed head as that candidate, and plans every required `unit:*` and `integration:*` proof that `scripts/contracts.mjs` registers. Head, base and policy revision come from the candidate record — for a queued candidate the base is its predicted base, not the base branch tip — and every other required proof is listed with the reason it waits for a producer session: `manual:*` and `e2e:*` proofs are never automatable, and an unregistered contract must reach `main` first. A fork, a branch outside `graphyard/*`, an item that has not submitted, or a head the control plane never observes plans nothing. A dispatch plans its single named proof from its inputs.
2. **exercise** runs one job per planned proof, named after the proof, in parallel and without any secret. `prepare-acceptance.mjs` refuses a head that no longer matches the pull request and fetches the candidate at the planned head merged with the planned base. An integration contract builds the candidate image and runs `run-acceptance.mjs` exactly as a dispatch does; a `unit` contract (`scripts/unit-contract.mjs`) installs the candidate's dependencies first, then copies the protected test file over the candidate's copy — byte-comparing it immediately before the run, so no install lifecycle script can put the candidate's version back — and runs its registered titles against the candidate's source with `run-unit-acceptance.mjs`, judging the fixed inventory from the TAP stream. Because a unit inventory shares a process with the code it tests, the lane certifies that the protected inventory passed, not that the candidate could not have interfered with its own test run; integration contracts judge from outside the container. Each job uploads an attempt- and proof-scoped report artifact.
3. **publish** (`graphyard-reporting`) downloads this attempt's reports and, for a candidate run, publishes each through the CI producer with `publish-acceptance.mjs --candidate-run`. The reporter reads the attempt-scoped run and its jobs from GitHub, refuses a run whose `head_sha` is not the candidate head or whose event is not `pull_request_target` or `workflow_dispatch`, requires exactly one completed job named after the proof with the conclusion the report claims, and submits the record with the run URL as its artifact and a `ciRun` binding naming that job. The control plane reads the job back through its own App and applies the [CI trust boundary](protocol/evidence.md#ci-produced-evidence) again — CI principal only, automatable families only, job on the evidence commit only, strictly newer attempt only — before accepting.

A queue tip Graphyard publishes for a candidate behind another is itself committed onto the pull-request branch (see the [merge queue](#merge-queue)), so the same `synchronize` trigger runs the plan on the exact tip the queue will land; a candidate already on its predicted base keeps its head as the tip and the run it already had.

Speed: `~/.npm` is cached through `setup-node`, the isolated `postgres:17-alpine` image is cached as a tarball, and the candidate image's layers are cached per pull request through the GHA build cache so a push that leaves `package-lock.json` alone skips `npm ci`. With warm caches a proof job for the current suite completes in under five minutes; a newer push to the same candidate cancels the older run. Every workflow is linted with a pinned, checksummed `actionlint` in required CI (`workflow-lint`).

Manual proofs stay producer sessions and the deploy smoke proof stays after delivery; the CI producer holds `unit:*` and `integration:*` and no other capability. The master loop's `--proof-workflow` dispatch keeps working for a proof that needs re-running without a push.
...
Only a recorded commit means the provider may have been called. A broker that stops after `merge-verify` succeeded but before `merge-commit` leaves a verified, uncommitted execution; the owning coordinator's next attempt resumes it rather than abandoning it, rebuilding the verification from the record (its `verifiedAt` and bounded clock offset) because the engine refuses a second verification of the same execution, and then runs the same clock wait, pre-commit revalidation, transactional commit and pre-provider checks as a first attempt. A resumed execution without enough lifetime left is cancelled so a fresh one can be acquired.
