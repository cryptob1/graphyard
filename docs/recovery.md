<!-- page: Build integrations | 5 | runner capacity, artifacts, rollback. -->
...
Fencing `provider` conditions the provider write on the expected running release; `serialized` freezes the environment until the operation settles; `none` is never automatic. The human operator, or a promoter's `delegate` lease, requests `POST /api/delivery/rollback` to a release that verified here before:
...
It performs the provider write and reports `applied`, `failed` or `unknown` with `POST /api/delivery/rollback-settle`:
