...
## Agent account capacity

A worker, reviewer or producer session that stops on its provider's usage-limit notice is not something to recover by hand: the master loop reads the notice from the session's output, commits the attempt's uncommitted work on its branch, records the exhaustion with the account and its reset time, holds the account until then, and launches the same action on another account within two minutes. When every account of a role is spent, each waiting item carries one capacity escalation naming the accounts and their resets, `master status` shows one `capacity` line, the role is not launched until the first reset, and nothing that needs a different role waits. An interrupted attempt's work is at the commit its `capacity.exhausted` history entry names (`partialWork.commit`, on `partialWork.branch` in `partialWork.path`). See [exhaustion in the middle of a session](master-agent-sessions.md#exhaustion-in-the-middle-of-a-session).
