...
Fencing `provider` conditions the provider write on the expected running release; `serialized` freezes the environment until the operation settles; `none` is never automatic. The operator, or a promoter's `delegate` lease, requests `POST /api/delivery/rollback` to a release that verified here before:
