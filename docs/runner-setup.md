...
The acknowledgement is the one transition that makes the attempt hold its reservations, and its response can be lost: the request times out, the connection drops after the control plane has already committed, or a gateway answers 408, 429 or a 5xx for a request the server may or may not have seen. Giving up on that first ambiguous answer would leave an attempt that is `running` on the server with no runner executing or heartbeating it, so it expires holding its runner, environment and external reservations until an operator settles it. The runner therefore treats an ambiguous answer as *unknown*, not as failure, and resolves it under this contract:

- **One acknowledgement, however many sends.** Every send carries the request key `ATTEMPT_ID-ack` and an identical body (`requestId`, `attemptId`, `epoch`). The control plane's idempotency receipt turns them into a single commit: a send that arrives after the commit replays the committed request document instead of acknowledging again, and a send with the same key but a different body is refused outright.
- **Confirmation is read from the answer, not from the status code.** Success means the returned request is `running` and its current attempt is this attempt, with an `acknowledgedAt`. A replayed receipt satisfies exactly that, so an already-acknowledged answer is success; a 2xx that shows anything else is refused without a retry, because sending again cannot make it this attempt's acknowledgement.
- **A confirmed refusal is a decision.** An expired, cancelled or superseded lease, or the wrong runner principal, is surfaced at once and never retried. Only transport failures, timeouts, 408, 429 and 5xx answers are retried.
- **The bound.** At most **5 sends** in total, paused 1, 2, 4 and 8 seconds apart, and no send starts more than **60 seconds** after the first — the lease a committed acknowledgement grants, beyond which a replay could only be refused as expired. Each send has the CLI's 30-second request timeout, and a dispatched attempt must be acknowledged within 30 seconds of dispatch, so an acknowledgement that never committed is refused as expired on the retry rather than committed late. When the bound is exhausted the command fails naming the last ambiguous answer; the attempt was either never acknowledged and expires releasing its reservations, or was acknowledged and expires as `running` for an operator to settle — `graphyard validation capacity` shows which, with its next step.
- **Nothing starts before confirmation.** No heartbeat is sent and the attestor is not told to proceed until the acknowledgement is confirmed, so an attempt whose acknowledgement stays ambiguous never exercises the target. The attestor's own re-read of Graphyard remains the authority to execute, as described under [the host attestor](#the-attestor-verifies-authority-itself).
