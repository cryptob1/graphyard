...
`master status` at startup and after events; `master run` dispatches ready work in `schedule.order`; merge exact candidates passing every gate, routing findings to rework; `master verify-deployment GY-N` after delivery. Close finished agent sessions; return to status.
...
Every `run.doctor.intervalMinutes` (10 by default) the loop launches the **doctor**: a headless session on the registry's `doctor` role (else Pi per `run.doctor`), with the master's read access and the operator-agent identity's sanctioned commands only — `master scope`, `requirements`, `unblock`, `decide` + `approver`, `settle-containment`, `close`, `create`, `release`; never merge, dispatch, evidence or leases. Findings, actions and filings show in `master status` and the dashboard's Doctor panel; unactable findings raise attention or file P0/P1, deduplicated. Off: `run.doctor.enabled=false`.
