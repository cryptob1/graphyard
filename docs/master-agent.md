...
Default on (`run.research`: `model`, `timeoutMinutes` 15, `tokenBudget`, `enabled: false`), a feature (or `"research": true`) gets one read-only Pi briefing per revision. Product questions: Needs you; build proceeds on the recommendation, a differing answer requests rework, failure never blocks.
