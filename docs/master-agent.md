...
The rules also cover everything else the master owns, so no routine master action waits for a human: reading and editing its own configuration (`.graphyard/master.json`); restarting, starting, stopping and reading the durable loop's user unit (`systemctl --user restart graphyard-master…`, `journalctl --user -u graphyard-master…`); deployment administration (`railway status`, `logs`, `deployment`, `redeploy`, and `curl` of the configured `run.deploymentUrl`); and CI runs (`gh run list`, `view`, `watch`, `rerun`, plus `gh workflow run` of the configured proof and smoke workflows).
