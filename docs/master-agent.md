...
`master status` reads those records under `executors`, beside the presence and supervision above, one line per executor with its commit beside
...
systemctl --user enable --now graphyard-executor@1 graphyard-executor@2
```
...
reviewer about a head until the unit and integration proofs its criteria name have passed on it:

- **Proofs pending.** A submitted, observed head gets its producer requests at once and no review
  request. `reviewNeed` reports `proofs-pending`, naming each criterion and proof that has not
  passed yet, and the next action is the producers' dispatch, not a review. The same hold stops the
  control plane's own `codex`/`agent` dispatch, so no provider is asked.
...
3. Compare the session's command line (`herdr pane read`, or the line the launcher typed) with the table above. On Codex, PATH is granted only if some `--add-dir DIR` on that line resolves to it: from the worktree, `test "$(realpath -m DIR)" = "$(realpath -m PATH)"` exits 0 for the `DIR` that grants PATH and 1 for any other, and when no `--add-dir` passes it PATH is not granted. The launcher appends its `--add-dir` grants only when the arguments select `--sandbox workspace-write` (or `-s workspace-write`), and only for paths not already granted; a profile's own `--add-dir` and `-c sandbox_workspace_write.*` flags stay on the line beside them, and a profile selecting any other sandbox mode, or none, gets no generated grant.
4. Reproduce outside the worker: from the worktree, `codex sandbox -P probe -C . -c 'permissions.probe.filesystem={":workspace_roots"="write", "/"="read"}' -- touch PATH/probe`, with `"DIR"="write"` added for each absolute directory the session's line grants with `--add-dir` (never PATH itself unless the line granted it), exits 1 with `Read-only file system` when PATH is not granted. Adding `"PATH"="write"` to that profile makes the same command exit 0, which confirms the missing grant is the fault; plain `touch PATH/probe` exiting 0 outside the sandbox confirms it is the sandbox policy, not the filesystem. Remove `PATH/probe` afterwards.
...
- `master status` lists them under the row's `reviewThreads` — each with its GraphQL node `id`, the argument `scripts/resolve-thread.mjs` takes — reports the row with `mergeable: false`, and its attention names the remedy: `master decide GY-N rework 'Address the unresolved review threads: …'`. A thread's path and author are text a contributor chose, so the reason is a single shell-quoted argument: running the command as printed cannot expand or execute anything a file name contains;
