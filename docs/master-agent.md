...
11. a server-derived elapsed-time wait until the next provider timestamp boundary plus a second execution-lifetime check, so host clock skew cannot shorten the interval and Graphyard can prove final verification preceded GitHub's whole-second merge interval;
