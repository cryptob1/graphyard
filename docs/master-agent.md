...
**Effective concurrency.** `master status` reports, under `concurrency`, how many items the
...
| `master status` | Work truth, session health, reviews, queue, `schedule` (dispatch order, overlap holds with their age and chain, overdue holds, high-conflict scopes), `concurrency` (effective concurrency beside idle workers), per-candidate `conflicts`, per-row `dispatch` (requested reviews and producers), per-row `merged` (an observed merge no execution authorized, with its recovery), `disk` (free space and what a reclaim would return), and `administration` (recent browser actions, pending sudo code) |
