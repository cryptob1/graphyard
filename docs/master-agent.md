...
With `run.research` set (`model`, `timeoutMinutes` 15, `tokenBudget`), a feature gets one read-only Pi briefing per revision. Product questions: Needs you; build proceeds on the recommendation, a differing answer requests rework, failure never blocks.
...
The repair lane, first of two audited no-admin-bypass uses: once a `"repair": "merge-path"` item (`mergePath` files only) stalls 15 minutes with checks passed and an approver agent's `master decide GY-N repair-merge REASON` naming the fault, the bypass merges its head, audited (`repair.merged`), flagged until a normal merge.
