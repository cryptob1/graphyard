...
One follow-up item per parent gains each approval's new findings; `followups/migrate` merged older duplicates once. With `run.research`, Pi triages follow-ups and `Recurring` items: release with a priority, close (fixed, or not worth it) or merge; closures need an approved `close` decision. Untriaged past 24h is attention; status counts `machineUntriaged` apart from `operatorBacklog`.
