...
3. Merge all-gates-passing candidates; route findings to rework.
...
**Concurrency is per role.** A profile's `concurrency` (1–20, default 1) caps its simultaneous sessions (names unique per request above one), applied without restart; lowering it drains sessions first (`longestWaitMs`); a role starved ten minutes counts in `counts.concurrencyStarved`.

**Launches bind the head, not observation age**; merges and rework need a two-minute-fresh one. `dispatch.waiting` has `waitedMs`, `reason`; reviews waiting 15+ minutes raise attention.
...
A pass is trusted only if that stripped run failed executing a case; otherwise it is recorded as not exercising its criterion rather than as passing (`unexercised`, `evidence.exercise.refused`); the loop requests rework quoting it. `decide attest` adds `exercise` (fails on base), approver-confirmed; unexercised `manual:` proofs: re-attest, never rework.
