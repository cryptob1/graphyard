...
`relevant.services` must name every environment service. A changed relevant path forbids reuse; a path matching neither list is **unknown, and unknown refuses**. `identical` requires the same artifact manifest; `scoped` allows another when build inputs are unchanged and every change is ignorable.
...
Refused, with every reason, unless the newest attempt is a fresh settled pass with the same pinned revisions and base and no relevant or unknown change; a grant's evidence carries a `reuse` block expiring at the freshness bound.
