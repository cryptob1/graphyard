...
A trusted run executes the contract from the protected checkout, so a new contract must be merged to `main` before any work item may require its proof; until then the pull request introducing it is covered by the unprivileged `container-recovery` CI job, which runs the identical inventory against the candidate and publishes no evidence. Dispatch a trusted run as described in [repository bootstrap](first-pr.md#bootstrap-sequence).
