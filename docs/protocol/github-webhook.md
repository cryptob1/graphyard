...
When a submitted candidate passes the build gate, `autoDispatch` records one `producers` request per proof group and, once the head's unit and integration proofs pass, a `review` request, each with `id`, `kind`, `sha`, `baseSha`, `policyRevision`, `pr`, `requestedAt`, `reason` and `state`. A request is `satisfied` by an approval or trusted evidence and `cancelled` when the head, base or policy changes, rework is requested or the PR closes; transitions append `dispatch.requested`, `dispatch.satisfied` or `dispatch.cancelled`, and resolved requests move to `autoDispatch.history`. Nothing here moves a gate.
