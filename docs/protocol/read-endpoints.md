...
- `GET /api/events?work=UUID`: Event rows for one item, newest first, narrowed by `kind` (`?kind=evidence,rework`) and shaped by `payload` (`?payload=full|details|none`); see [an item's history](#reading-an-items-history)
