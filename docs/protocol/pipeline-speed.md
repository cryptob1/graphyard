...
The top-level `speed` of the same status is the [periodic measurement](../master-agent-reference.md#pipeline-speed): nearest-rank p50/p90 of `submitToMergeMs` over every measured delivery and over the routine ones, the rework-round median and distribution, hand-off counts, execution share, and the target verdict. Its `items` report execution versus wait, rework rounds and hand-offs per delivered item, and its `coverage` accounts for every delivery in the window: how many are measured, how many are `awaitingBackfill`, `eventsPruned` or `noSubmission`, the key and reason of each one that is not, `complete` when none is left, and a `statement` saying so. An unmeasured delivery is therefore named, never silently absent.
