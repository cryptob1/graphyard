...
- `attempts` — one entry per lease epoch. `claim` opens it; `submit` (`end: submitted`), `release` (`released`), a lapse that reconciliation, a replacement claim or a requirements revision records (`expired`, at the lease's own deadline) or `rework` of a live lease (`reworked`) closes it. An attempt still open when a later claim arrives is closed as `expired`.
...
`graphyard master status` reports, on every open and delivered row under `speed`, what `src/pipeline-speed.ts` derives from the timeline: `executionMs` (lease time summed over attempts, the active one counting up to now), `waitMs` (`openMs` from the first claim to the accepted merge or now, minus `executionMs`), `submitToMergeMs` (first submission to the accepted merge on the repository clock — `delivery.mergedAtRepository`, else `mergedAt` — `null` until delivered), `sinceSubmitMs` while in flight, `reworkRounds` and `interventions` as recorded, and `routine` (at most one rework round and no intervention). The top-level `speed` is the [periodic measurement](../master-agent.md#pipeline-speed) over every measured delivery.
