...
- **`Idempotency-Key`:** required on every mutation, at most 200 characters. Generate a UUID once and reuse it only when retrying the identical request after a timeout.
