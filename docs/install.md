...
`graphyard doctor --profile through-merge|preview-validation|production-verification` prints an explicit checklist for the selected [completion profile](turnkey-delivery-roadmap.md#product-promise-and-boundary).
...
4. Nothing else is needed: the next preflight releases the held jobs without a restart; `doctor` confirms `appPermissions.missing` is empty and `heldJobs` is `0`.
