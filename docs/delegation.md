...
| --- | --- |
| `lease-loss` raised by the control plane (actor `graphyard`) for an epoch whose lapse the ledger explains | Reconciliation, automatically (above); or any `admin` principal, whatever its declared session kind, with `resolve GY-N lease-loss --attestation blocked\|stopped-worker "reason"`. The server verifies the citation against the ledger — the kind named must exist for the exact epoch the standing escalation names — and refuses `The ledger holds no KIND attestation for epoch N` otherwise. A declared human session may also resolve it, with or without a citation. |
