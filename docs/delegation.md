...
| Trigger | Raised when |
| --- | --- |
| `lease-loss` | Reconciliation observes an expired worker lease on an epoch with no bound submission, a replacement claim takes over one, or operator rework discards one. A lease that lapses after `submit` is a `lease.expired` history entry, not an escalation. |
...
Lease loss is recorded wherever an unfinished assignment is discarded, not only where it expires: by reconciliation when a lease runs out on an epoch with no bound submission, by the replacement claim itself, which records the expired owner and epoch in the same transaction that overwrites the lease, and by operator `rework`, which clears a still-held lease outright. Rework is the last point at which that loss is visible — reconciliation afterwards sees no lease to expire, and the replacement claim reads a lease that is already `null` — so the incident is recorded there against the epoch it belonged to, with `lastAssignment` still naming the worker that held it. The operator's `previousWorkerStopped` attestation authorizes the reassignment; it does not decide whether the incident is recorded. Rework that discards no lease raises nothing. A replacement can therefore never erase the evidence that an assignment was lost, and the replacement worker may claim and work the reopened item while the escalation stands, though delivery waits on a declared human session resolving it.

A submission is the end of its lease, not a loss of it. `submit` releases the implementation lease in the same transaction that binds the candidate, so a worker that has completed holds nothing for a later expiry or rework to discard, and a lease that still lapses under a submitted epoch — one bound before this rule, or a worker still renewing when its renewal was refused — is recorded by reconciliation as a `lease.expired` history entry with no escalation. Standing `lease-loss` escalations whose epoch already has a bound submission are settled by reconciliation itself, on deploy and on every later tick, with an `escalation.auto-settled` history entry carrying the note `auto-settled: submitted before expiry`; the `resolve` command, and its declared-human-session requirement, are untouched for every other escalation.
...
Lease loss reaches the record the same way: reconciliation records an expired lease, and raises its escalation for an unsubmitted epoch, even while a merge execution is active, rather than deferring until the execution ends.

## Ownership and worktrees

Ownership is bound per claimed item, not per worker. Every claimed item is held by exactly one worker identity under one lease epoch, is implemented in exactly one registered worktree for that assignment, and stays claimed only while its `watch` supervisor keeps renewing the lease. The server enforces each part of that binding: a second identity claiming an owned item is refused with `Task already has an active owner`; a heartbeat, workspace, submission, or release from any other identity or epoch is refused with `Lease missing, expired, or superseded`; a second worktree for the same assignment is refused with `This assignment already has a workspace`; and a worktree path or branch that another assignment has already registered — including another item held by the same engineer — is refused as reserved or overlapping. When the supervisor stops renewing before the attempt has submitted, reconciliation clears the lease on the server's clock, records a `lease-loss` escalation that only a declared human session can resolve, keeps the lost owner and epoch in `lastAssignment`, and leaves the item claimable by a replacement under a new epoch. Once the attempt has submitted, the lease has already ended with `complete`: a renewal for that epoch is refused with `Implementation lease for epoch N ended when GY-N was submitted; stop heartbeating after complete`, the supervisor stops the session, and nothing escalates.
