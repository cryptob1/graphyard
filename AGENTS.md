...
- **Before submitting domain or API changes:** `npm run build` and `npm test`; tests run a temporary real Postgres database, never production data
- **When behaviour changes:** update the relevant guide under `docs/`, and keep external I/O outside coordination transactions
- **Secrets:** no secret belongs in Git; `.graphyard/credentials.json` and `.env` are local-only
- **Deployment changes:** the Dockerfile, `compose.yaml`, `deploy/helm/graphyard` and `.railway/railway.ts`; preview infrastructure changes before applying, and keep the release contract (`scripts/verify-image-release.mjs`) and the chart exercise passing
