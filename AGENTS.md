...
Run `npm run build` and `npm test` for domain or API changes; tests run a temporary real Postgres database, never production data. Update the relevant guide under `docs/` when behaviour changes, and keep external I/O outside coordination transactions. No secrets belong in Git: `.graphyard/credentials.json` and `.env` are local-only. Deployment changes use the Dockerfile, `compose.yaml`, `deploy/helm/graphyard` and `.railway/railway.ts`; preview infrastructure changes before applying, and keep the release contract (`scripts/verify-image-release.mjs`) and the chart exercise passing.
